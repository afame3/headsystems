...
class CommunityControllerCommunity extends CommunityController
{
	...
	{	
		...
		if( $model )
		{
			...
			//Set Groups
			$polls	= $this->getModel( 'Polls' ,'CommunityAdminModel');
			$view->setModel( $polls  , false );
		}

		// Set the layout
		$view->setLayout( $layout );

		// Check if default ACL permissions are stored
		$rules = json_decode(JAccess::getAssetRules('com_community'), true);
