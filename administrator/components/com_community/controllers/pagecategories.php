...
class CommunityControllerPageCategories extends CommunityController
{
	...
	public function ajaxSaveCategory($data)
	{
		...
		$row = JTable::getInstance('PageCategory', 'CTable');
		$row->load($data['id']);
		...
		if (!trim($row->name)) {
			$response->addAlert(JText::_('COM_COMMUNITY_PAGE_EMPTY_NAME_ERROR'));
		...
		{
			$response->addScriptCall('azcommunity.redirect', JURI::base() . 'index.php?option=com_community&view=pagecategories');
		}
	...
	public function ajaxEditCategory($id)
	{
		...
		$row = JTable::getInstance('pagecategories', 'CommunityTable' );
		$row->load($id);
...
?>
		...
		<div class="alert notice">
			<?php echo JText::_('COM_COMMUNITY_PAGES_CATEGORY_DESC');?>
		</div>
...
<?php
		...
		$buttons	= '<input type="button" class="btn btn-small btn-primary pull-right" onclick="javascript:azcommunity.savePageCategory();return false;" value="' . JText::_('COM_COMMUNITY_SAVE') . '"/>';
	...
	public function removecategory()
	{
		...
		foreach ($ids as $id) {
			if (!$row->delete($id)){
				...
				$message = JText::_('COM_COMMUNITY_PAGES_ASSIGNED_CATEGORIES');
				$mainframe->redirect( 'index.php?option=com_community&view=pagecategories' , $message , 'error');
				exit;
			}
		}
		$message = JText::sprintf( 'COM_COMMUNITY_CATEGORY_SUCCESSFULLY_REMOVED_COUNT' , $count );
		$this->cacheClean(array(COMMUNITY_CACHE_TAG_PAGES_CAT));
		$mainframe->redirect( 'index.php?option=com_community&view=pagecategories' , $message ,'message' );
	}
}