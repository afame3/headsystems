...
abstract class CommunityDefaultItem
{
	...
	/**
	 * Add default Page Categories
	 */
	static public function addDefaultPagesCategories()
	{
		$db		= JFactory::getDBO();

		$query 	= 'INSERT INTO ' . $db->quoteName('#__community_pages_category') . ' (' . $db->quoteName('id') .', ' . $db->quoteName('name') .', ' . $db->quoteName('description') .') VALUES ' .
					'(' . $db->Quote('1') .', ' . $db->Quote('General') .', ' . $db->Quote('General page category.') .'),'.
					'(' . $db->Quote('2') .', ' . $db->Quote('Internet') .', ' . $db->Quote('Internet page category.') . '),'.
					'(' . $db->Quote('3') .', ' . $db->Quote('Business') .', ' . $db->Quote('Business page category') .'),'.
					'(' . $db->Quote('4') .', ' . $db->Quote('Automotive') .', ' . $db->Quote('Automotive page category') .'),'.
					'(' . $db->Quote('5') .', ' . $db->Quote('Music') .', ' . $db->Quote('Music page category') .')';
	...
	static public function addDefaultUserPoints()
	{
		...
		$query = "INSERT INTO `#__community_userpoints` (`id`, `rule_name`, `rule_description`, `rule_plugin`, `action_string`, `component`, `access`, `points`, `published`, `system`) VALUES
					...
					(60, '
						Following', 'Add points when user following someone', 'com_community', 'followers.add', '', 1, 1, 1, 1), 
					(61, '
						Unfollow', 'Deduct points when user unfollow someone', 'com_community', 'followers.remove', '', 1, 1, -1, 1),
                    (62, 'New Page Created', 'Assign number of points when a user creates a new page and determines if an activity stream item will be created.', 'com_community', 'page.create', '', 1, 3, 1, 1),
                    (63, 'Like page', 'Add points when user like a page', 'com_community', 'pages.like', '', 1, 1, 1, 1),
                    (64, 'Unlike page', 'Deduct points when user unlike a page', 'com_community', 'pages.unlike', '', 1, -1, 1, 1),
                    (65, 'Change Page Avatar', 'Assign a number of points when a page admin or owner changes its avatar.', 'com_community', 'page.avatar.upload', '', 1, 0, 1, 1)";
		$db->setQuery( $query );
		try {
			$db->execute();
		} catch (Exception $e) {
	...
	static public function addDefaultCustomFields()
	{
		...
		$query	= 'INSERT INTO ' . $db->quoteName('#__community_fields') . ' (' . $db->quoteName('id') . ', ' . $db->quoteName('type') . ', ' . $db->quoteName('ordering') . ', ' . $db->quoteName('published') . ', ' . $db->quoteName('min') . ', ' . $db->quoteName('max') . ', ' . $db->quoteName('name') . ', ' . $db->quoteName('tips') . ', ' . $db->quoteName('visible') . ', ' . $db->quoteName('required') . ', ' . $db->quoteName('searchable') . ', ' . $db->quoteName('options') . ', ' . $db->quoteName('fieldcode') . ') VALUES '.
					...
					'(' . $db->Quote('5') . ', ' . $db->Quote('location') . ', ' . $db->Quote('5') . ', ' . $db->Quote('1') . ', ' . $db->Quote('1') . ', ' . $db->Quote('100000') . ', ' . $db->Quote('Location') .', ' . $db->Quote('Your Location') . ', ' . $db->Quote('1') . ', ' . $db->Quote('1') . ', ' . $db->Quote('1') . ', ' . $db->Quote('') . ', ' . $db->Quote('FIELD_LOCATION') . '),' .
					'(' . $db->Quote('6') . ', ' . $db->Quote('group') . ', ' . $db->Quote('6') . ', ' . $db->Quote('1') . ', ' . $db->Quote('10') . ', ' . $db->Quote('100') . ', ' . $db->Quote('Contact Information') . ', ' . $db->Quote('Specify your contact details') . ', ' . $db->Quote('1') . ', ' . $db->Quote('1') . ', ' . $db->Quote('1') . ', ' . $db->Quote('') . ', ' . $db->Quote('') . '),' .
					'(' . $db->Quote('7') . ', ' . $db->Quote('text') . ', ' . $db->Quote('7') . ', ' . $db->Quote('1') . ', ' . $db->Quote('10') . ', ' . $db->Quote('100') . ', ' . $db->Quote('Mobile phone') .', ' . $db->Quote('Mobile carrier number so other users can contact you.') . ', ' . $db->Quote('1') . ', ' . $db->Quote('0') . ', ' . $db->Quote('1') . ', ' . $db->Quote('') . ', ' . $db->Quote('FIELD_MOBILE') . '),' .
					'(' . $db->Quote('8') . ', ' . $db->Quote('text') . ', ' . $db->Quote('8') . ', ' . $db->Quote('1') . ', ' . $db->Quote('10') . ', ' . $db->Quote('100') . ', ' . $db->Quote('Land phone') . ', ' . $db->Quote('Contact number so other users can contact you.') . ', ' . $db->Quote('1') . ', ' . $db->Quote('0') . ', ' . $db->Quote('1') . ', ' . $db->Quote('') . ', ' . $db->Quote('FIELD_LANDPHONE') . '),' .
					'(' . $db->Quote('9') . ', ' . $db->Quote('url') . ', ' . $db->Quote('9') .', ' . $db->Quote('1') .', ' . $db->Quote('10') .', ' . $db->Quote('100') .', ' . $db->Quote('Website') .', ' . $db->Quote('Your website') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('') .', ' . $db->Quote('FIELD_WEBSITE') . '),' .
					'(' . $db->Quote('10') . ', ' . $db->Quote('group') . ', ' . $db->Quote('10') .', ' . $db->Quote('1') . ', ' . $db->Quote('10') .', ' . $db->Quote('100') .', ' . $db->Quote('Education') .', ' . $db->Quote('Educations') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('') . ', ' . $db->Quote('') .'),' .
					'(' . $db->Quote('11') . ', ' . $db->Quote('text') .', ' . $db->Quote('11') .', ' . $db->Quote('1') .', ' . $db->Quote('10') .', ' . $db->Quote('200') .', ' . $db->Quote('College / University') .', ' . $db->Quote('Your college or university name') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('') .', ' . $db->Quote('FIELD_COLLEGE') . '),' .
					'(' . $db->Quote('12') . ', ' . $db->Quote('text') . ', ' . $db->Quote('12') .', ' . $db->Quote('1') .', ' . $db->Quote('5') .', ' . $db->Quote('100') .', ' . $db->Quote('Graduation Year') .', ' . $db->Quote('Graduation year') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('1') .', ' . $db->Quote('') .', ' . $db->Quote('FIELD_GRADUATION') .')';

		$db->setQuery( $query );
		$db->execute();

		$hide_ids = array(5,7,8,9,10,11,12);

		$query= "UPDATE  "
			. $db->quoteName('#__community_fields')
			. " SET " 	. $db->quoteName('required') 	. "=" . $db->quote(0)
