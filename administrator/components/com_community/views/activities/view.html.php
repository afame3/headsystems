			...
			switch ($activity->app) {
				...
				case 'profile':
						...
						elseif($activity->pageid)
						{
							...
							$html .= '➜ <a class="cStream-Reference" target="_blank" href="'.JUri::root().'index.php?option=com_community&view=pages&task=viewpage&pageid='.$page->id.'">'.$page->name.'</a>';
						}
				...
				case 'pages.avatar.upload':
						$user = CFactory::getUser($activity->actor);
						$html = '<a class="cStream-Author" target="_blank" href="'.JUri::root().'index.php?option=com_community&view=profile&userid='.$user->id.'">'.$user->getDisplayName().'</a> ';
						$html .=	JText::_('COM_COMMUNITY_ACTIVITIES_NEW_PAGE_AVATAR');
						$activities[$key]->title = $html;
					break;
				...
				case 'album.like':
						...
						switch($activity->app){
							...
							case 'pages.like':
								...
								$urlLink 	= JUri::root().'index.php?option=com_community&view=pages&task=viewpage&pageid='.$cid->id;
								$name 		= $cid->name;
				...
				case 'cover.upload':
						...
						if(strtolower($type) !=='profile')
						{
							...
							if($type == 'page')
							{
								$extraMessage = ' <a target="_blank" href="'.JUri::root().'index.php?option=com_community&view=pages&task=viewpage&pageid='.$cTable->id.'">'.$cTable->name.'</a>';
							}
							if($type == 'group')
							{
				...
				case 'pages.update':
						...
						$html = '<a class="cStream-Author" target="_blank" href="'.JUri::root().'index.php?option=com_community&view=profile&userid='.$user->id.'">'.$user->getDisplayName().'</a> - '. JText::sprintf('COM_COMMUNITY_PAGES_PAGE_UPDATED' , JUri::root().'index.php?option=com_community&view=pages&task=viewpage&pageid='.$page->id , $page->name );
						$activities[$key]->title =$html;
					break;
				case 'photos':
						$user = CFactory::getUser($activity->actor);
