 ...
 */
class CommunityViewPages extends JViewLegacy
{
    /**
    ...
    public function display( $tpl = null )
    {
        ...
        JToolBarHelper::title( JText::_('COM_COMMUNITY_PAGES'), 'pages');

        // Add the necessary buttons
        JToolBarHelper::deleteList( JText::_('COM_COMMUNITY_PAGE_DELETION_WARNING') , 'deletePage' , JText::_('COM_COMMUNITY_DELETE') );
        ...
        // We need to assign the users object to the pages listing to get the users name.
        ...
        $filter_order       = $mainframe->getUserStateFromRequest( "com_community.pages.filter_order",     'filter_order',     'a.name',   'cmd' );
        $filter_order_Dir   = $mainframe->getUserStateFromRequest( "com_community.pages.filter_order_Dir", 'filter_order_Dir', '',         'word' );
    ...
    public function _displayEditLayout( $tpl )
    {
        ...
        // Add the necessary buttons
        JToolBarHelper::back(JText::_('COM_COMMUNITY_BACK') , 'index.php?option=com_community&view=pages');
        JToolBarHelper::divider();
        JToolBarHelper::save();

        $page  = JTable::getInstance( 'Page' , 'CTable' );
        $page->load( $jinput->getInt( 'pageid') );
    ...
    public function getThumbAvatar($obj) {
        ...
        // For page avatars that are stored in a remote location, we should return the proper path.
        ...
        $thumb = CUrlHelper::avatarURI($obj->thumb, 'page_thumb.png');
