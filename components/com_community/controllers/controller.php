...
class CommunityBaseController extends JControllerLegacy
{
    ...
    public function execute($task)
    {
        ...
        $config = CFactory::getConfig();
        $viewName = CStringHelper::strtolower($jinput->request->get('view', '', 'STRING'));
        ...
        $reaction = $config->get('enablereaction') ? 'joms-reaction--on' : 'joms-reaction--off';

        $html = '<div id="community-wrap" class="' . $reaction . ' jomsocial-wrapper on-' . $env->joomlaTemplate . ' ' . $document->direction . ' c' . ucfirst($viewName) . '">';
        ...
        // Build toolbar HTML
        ob_start();
        $view = $this->getView(CStringHelper::strtolower($jinput->get('view', 'frontpage', 'STRING')));
