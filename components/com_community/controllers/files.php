...
class CommunityFilesController extends CommunityBaseController {
    ...
    public function multiUpload() {
        ...
        $parentTable && $parentTable->load($id);
        ...
        $file->name = CStringHelper::substr($_file['name'], 0, CStringHelper::strlen($_file['name']) - (CStringHelper::strlen($ext['extension']) + 1));
        $file->created = $now->toSql();
        $file->type = CFileHelper::getExtensionIcon(CFileHelper::getFileExtension($_file['name']));
        $fileName = JApplicationHelper::getHash($_file['name'] . time()) . CStringHelper::substr($_file['name'], CStringHelper::strlen($_file['name']) - (CStringHelper::strlen($ext['extension']) + 1));
        ...
        switch ($type) {
            ...
            case 'groupchat':
                ...
                break;
            case 'activities':
                $file->filepath = 'images/files/' . $type . '/' . $my->id . '/' . $fileName;
                $file->actid = -1;
                if (!JFolder::exists(JPATH_ROOT . '/images/files/' . $type . '/' . $my->id)) {
                    JFolder::create(JPATH_ROOT . '/images/files/' . $type . '/' . $my->id);
                    JFile::copy(JPATH_ROOT . '/components/com_community/index.html', JPATH_ROOT . '/images/files/' . $type . '/' . $my->id . '/index.html');
                }
        ...
        if(!in_array( $type, array('message', 'chat', 'groupchat', 'activities'))){
        ...
        $json = array('id' => $table->id, 'nane' => $table->name, 'path' => JUri::root().$table->filepath);
        die(json_encode($json));
    }
    ...
    public function downloadFile() {
        ...
        if ($type == 'activity') {
            if (!$my->authorise('community.download', 'files.' . $type, $cTable->actid)) {
                $mainframe->enqueueMessage(JText::_('COM_COMMUNITY_PERMISSION_DENIED_WARNING'), 'error');
                $mainframe->redirect(CRoute::_('index.php?option=com_community' . $groupLink, false));
                exit;
            }
        } else {
