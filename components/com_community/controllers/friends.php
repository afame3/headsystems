...
class CommunityFriendsController extends CommunityBaseController
{
    ...
    public function friends()
    {
        ...
        if (!$friend->id) {
            $redirectUrl = CRoute::_('index.php?option=com_community', false);
        ...
        if ($jinput->post->get('action', '', 'STRING') == 'invite') {
            ...
            if (empty($saveSuccess) || !in_array(false, $saveSuccess)) {
                ...
                if (empty($emails)) {
                    ...
                    //before anything else, check if this is on invitation only or not, if yes, try to validate the limit if applicable
                    $config = CFactory::getConfig();
                    $maxInvitations = $config->get('invite_registation_limit', 0);

                    $invitationTable = JTable::getInstance('invitation', 'CTable');
                    $invitationTable->load($my->id);

                    // if max invitation is not unlimited, lets count the limitation
                    // super admin doesn't have max invitation restriction
                    if (!COwnerHelper::isCommunityAdmin()) {
                        if ($maxInvitations && $maxInvitations < ($invitationTable->getTotalInvitedUsers() + count($emails))) {
                            //throw an error here
                            $validated = false;
                            $mainframe->enqueueMessage(JText::_('COM_COMMUNITY_REGISTRATION_REACHED_MAXIMUM_INVITES'));
                        }
                    }
                ...
                if ($validated) {
                    ...
                    for ($i = 0; $i < count($emails); $i++) {
                        ...
                        $params->set('url', 'index.php?option=com_community&view=register&inv_id='.$my->id.'&email='.$emails[$i]);
                        $params->set('message', $message);
                        $params->set('displayName', $my->getDisplayName());
                    ...
                    //add user to the invitation table
                    $invitationTable->addUser($emails);
                    $invitationTable->store(); //update invitation table
                    ...
                    $jinput->post->set('emails', '');
                    $jinput->post->set('message', '');
    ...
    public function deleteSent()
    {
        ...
        if ($model->deleteSentRequest($my->id, $friendId)) {
            ...
            if($redirect != ''){
                $mainframe->redirect( $redirect, $message, 'message');
            }
    ...
    public function ajaxApproveRequest($requestId)
    {
        ...
        if ($friendsModel->isMyRequest($requestId, $my->id)) {
            ...
            if ($connected) {
                ...
                // add new following to both side
                $modelFollower = CFactory::getModel('followers');

                if (!$modelFollower->isFollowing($my->id, $friendId)) {
                    $modelFollower->addFollowing($my->id, $friendId);
                }

                if (!$modelFollower->isFollowing($friendId, $my->id)) {
                    $modelFollower->addFollowing($friendId, $my->id);
                }
                ...
                $json['success'] = true;
                $json['message'] = str_replace('_QQQ_', '"', JText::sprintf('COM_COMMUNITY_FRIEND_REQUEST_ACCEPTED', $friend->getDisplayName(), $friendUrl));
    ...
    public function ajaxSaveFriend($postVars)
    {
        ...
        if (!empty($connection) || !$my->authorise('community.request', 'friends.' . $id)) {
            die();
        }
    ...
    public function delete($id)
    {
        ...
        // remove following to both side
        $modelFollower = $this->getModel('followers');
        $modelFollower->unFollow($my->id, $friend->id);
        $modelFollower->unFollow($friend->id, $my->id);
