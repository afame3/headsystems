...
class CommunityGroupsController extends CommunityBaseController {
    ...
    public function ajaxRemoveMember($memberId, $groupId) {
        ...
        if (!$my->authorise('community.remove', 'groups.member.' . $memberId, $group)) {
            ...
            //add user points
            CUserPoints::assignPoint('group.member.remove', $my->id);
    ...
    public function ajaxSaveDiscussionWall($message, $uniqueId, $photoId = 0) {
        ...
        // Add activity ID to wall params
        if ($activity->id) {
            $updateWall = JTable::getInstance('Wall', 'CTable');
            $updateWall->load($wall->id);

            $params = new CParameter($updateWall->params);
            $params->set('activityid', $activity->id);

            $updateWall->params = $params->toString();
            $updateWall->store();
        }
    ...
    public function edit() {
        ...
        if ($jinput->getMethod() == 'POST') {
            ...
            if (empty($saveSuccess) || !in_array(false, $saveSuccess)) {
                ...
                if (empty(strip_tags($group->description))) {
                ...
                if (empty($saveSuccess) || !in_array(false, $saveSuccess)) {
                    ...
                    if ($gid !== FALSE) {
                        ...
                        if ($config->get('moderategroupcreation')) {
                            $mainframe->enqueueMessage(JText::sprintf('COM_COMMUNITY_GROUPS_MODERATION_MSG', $group->name), 'message');
                            $mainframe->redirect(CRoute::_('index.php?option=com_community&view=groups&task=viewgroup&groupid='.$group->id, false));
                            return;
                        }
    ...
    private function _bindParams() {
        ...
        if (array_key_exists('filesharingpermission-admin', $jinput->post->getArray())) {
            $params->set('filesharingpermission', GROUP_FILESHARING_PERMISSION_ADMINS);

            if (array_key_exists('filesharingpermission-member', $jinput->post->getArray())) {
                $params->set('filesharingpermission', GROUP_FILESHARING_PERMISSION_ALL);
            }
        } else {
    ...
    public function save() {
        ...
        if ($validated) {
            ...
            if ($group->published && !$group->unlisted) {
                ...
                $params->set('group_url', 'index.php?option=com_community&view=groups&task=viewgroup&groupid=' . $group->id);
                $params->set('category_url', 'index.php?option=com_community&view=groups&categoryid=' . $group->categoryid);
    ...
    public function mygroups() {
        ...
        if (!$user->id) {
            $redirectUrl = CRoute::_('index.php?option=com_community&view=groups', false);
    ...
    private function _saveDiscussion(&$discussion) {
        ...
        if (CFactory::getUser()->authorise('community.postcommentedit', 'com_community')) {
            // allow to edit
        } else if ($my->id != $creator->id && !empty($discussion->creator) && !$groupsModel->isAdmin($my->id, $discussion->groupid) && !COwnerHelper::isCommunityAdmin()) {
            $mainframe->enqueueMessage(JText::_('COM_COMMUNITY_ACCESS_FORBIDDEN'), 'error');
            return false;
        }
        ...
        if (empty($saveSuccess) || !in_array(false, $saveSuccess)) {
            ...
            if ($validated) {
                ...
                if (( preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $discussion->message))) {
                    $linkFetchcontent = strip_tags($discussion->message);
                    $graphObject = CParsers::linkFetch($linkFetchcontent);
        ...
        if ($jinput->getMethod() == 'POST') {
            ...
            if ($validated) {
                ...
                if (( preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $bulletin->message))) {
                    $linkFetchcontent = strip_tags($bulletin->message);
                    $graphObject = CParsers::linkFetch($linkFetchcontent);
                ...
                $act->content = ( $group->approvals == 0 ) ? CStringHelper::substr(strip_tags($bulletin->message), 0, 100) : '';
