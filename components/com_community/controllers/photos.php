...
// no direct access
    ...
    class CommunityPhotosController extends CommunityBaseController
    {
        ...
        public function ajaxUploadPhoto($albumId = '', $contextId = '', $context)
        {
            ...
            if ($contextId) {
                if ($context == 'page') {
                    $title = JText::_('COM_COMMUNITY_PAGE_PHOTOS_UPLOAD_PHOTOS');
                } else if ($context == 'event') {
                    $title = JText::_('COM_COMMUNITY_EVENT_PHOTOS_UPLOAD_PHOTOS');
                } else {
                    $title = JText::_('COM_COMMUNITY_GROUPS_PHOTOS_UPLOAD_PHOTOS');
                }
            }
...
//			$url = $handler->getEditedAlbumURL( $albumid );
            ...
            if(empty($album)) {
                ...
                switch ($type) {
                    ...
                    case PHOTOS_PAGE_TYPE:
                        $page = JTable::getInstance('Page', 'CTable');
                        $page->load($pageId);
                        $album->name = JText::sprintf('COM_COMMUNITY_PAGE_DEFAULT_ALBUM_NAME', $page->name);
                        break;
                }
                ...
                if ($type == PHOTOS_PAGE_TYPE) {
                    ...
                    CNotificationLibrary::add(
                        ...
                        $my->id,
                        $pageMembers,
                        JText::sprintf('COM_COMMUNITY_PAGE_NEW_ALBUM_NOTIFICATION'),
                        '',
                        'pages.album',
                        $params
                    );
        ...
        private function _checkUploadedFile($imageFile, $album, $handler, $commentImage = false)
        {
            ...
            if ($handler->isExceedUploadLimit(false) && !COwnerHelper::isCommunityAdmin()) {
                ...
                if (intval($pageId) > 0) {
                    ...
                    $this->_showUploadError(true, JText::sprintf('COM_COMMUNITY_PAGES_PHOTO_LIMIT', $photoLimit));
        ...
        */
        ...
        public function removecover() {
            ...
            // page
                ...
                if ($isAdmin || $isSuperAdmin || $isMine) {
                    ...
                    $message = JText::_('COM_COMMUNITY_REMOVE_PAGE_COVER_SUCCESS');
                }
        ...
        public function ajaxSetPhotoAlbum($photoIds)
        {
            ...
            foreach ($albums as $album) {
                ...
                if ( $album->name == 'Stream Photos' ||
                     $album->name == JText::_('COM_COMMUNITY_PAGE_DEFAULT_ALBUM_NAME') ||
            ...
            if ( count($filtered) < 1 ) {
                if ($currAlbum->pageid > 0) {
                    ...
                    $error = JText::sprintf('COM_COMMUNITY_NO_MORE_ALBUMS_ON_PAGE', $page->name);
                } else if ($currAlbum->groupid > 0) {
                    $group = JTable::getInstance('Group', 'CTable');
        ...
        public function ajaxConfirmPhotoAlbum($albumId, $photoIds)
        {
            ...
            if ($albumId <= 0) {
                $json = array( 'error' => JText::_('COM_COMMUNITY_MOVE_TO_ANOTHER_ALBUM_NO_ALBUM_SELECT') );
                die( json_encode( $json ) );
            }
