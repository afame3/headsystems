...
class CommunityRegisterController extends CommunityBaseController
{
    ...
    public function register_save()
    {
        ...
        /* Do not allow for user registration if applicable*/
        ...
        $invitorIdLink = '';
        //we still need to validate against the invitation table if this is invitation only registration
        if(($config->get('invite_only') || $config->get('invite_only_request')) && $usersConfig->get('allowUserRegistration') == '0'){
            $email = $jinput->post->get('jsemail','','RAW');
            ...
            if ($invitorId) {
                ...
                //if all validation is done, set the invitor id to session
                $mySess->set('invitor_id', $invitorId);
            }
        }
        ...
        if (count($fields) <= 0 && (!$profileTypes || !$config->get('profile_multiprofile'))) {
            ...
            if ($config->get('enableavataruploadregister')) {
    ...
    private function _createUser($tmpUser, $requireApproval = false, $profileType = 0)
    {
        ...
        if ($profileType != COMMUNITY_DEFAULT_PROFILE) {
            ...
            if ($useractivation == 0) $useractivation = $multiprofile->approvals;
        }
        ...
        if ($profileType != COMMUNITY_DEFAULT_PROFILE) {
            ...
            // assign new user to profile user groups
            $params = new CParameter('');
            $params->loadString($multiprofile->params);
            $userGroups = $params->get('userGroup', array());
            
            foreach ($userGroups as $usergroupid) {
                JUserHelper::addUserToGroup($cuser->id, $usergroupid);
            }

            $cuser->_usergroup = implode(',', $userGroups);
            ...
            foreach ($assignGroupids as $groupid) {
                ...
                if(!$group->isMember($data->memberid, null)){
                    ...
                    $appsLib = CAppPlugins::getInstance();
                    $appsLib->loadApplications();
                    $appsLib->triggerEvent('onGroupJoin', $group, $user->id);
                } else {
                    // approving member
                    $member = JTable::getInstance('GroupMembers', 'CTable');
                    $keys = array('groupId' => $groupid, 'memberId' => $cuser->id);
                    $member->load($keys);
                    $member->approve();
                }
            }
    ...
    private function _validateRegister($post = array())
    {
        ...
        $sitename = JFactory::getConfig();
        $sitename = $sitename->get( 'sitename' );

        if ($config->get('enableterms') && !$config->get('enablegdpr')) {
            if (empty($data['tnc'])) {
                $errMsg[] = JText::_('COM_COMMUNITY_REGISTER_ACCEPT_TNC');
            }
        } else if (!$config->get('enableterms') && $config->get('enablegdpr')) {
            if (empty($data['tnc'])) {
                $errMsg[] = JText::sprintf('COM_COMMUNITY_REGISTER_ACCEPT_AGREE_GDPR_ERROR', $sitename);
            }
        } else if ($config->get('enableterms') && $config->get('enablegdpr')) {
            if (empty($data['tnc'])) {
                $errMsg[] = JText::_('COM_COMMUNITY_REGISTER_ACCEPT_TNC') . ' ' . JText::sprintf('COM_COMMUNITY_REGISTER_ACCEPT_AGREE_GDPR_ERROR_AND', $sitename);;
            }
        }
        ...
        if (!empty($data)) {
            foreach ($groups as $group) {
                ...
                for ($i = 0; $i < count($fields); $i++) {
                    ...
                    if ($isRequired == 1) {
                        if ($fieldType == 'date') {
                            if (CStringHelper::trim($data['field' . $fieldid][0]) == '' || CStringHelper::trim(
    ...
    public function ajaxCheckUserName($username = '', $current = '')
    {
        ...
        if (!empty($username)) {
            if (!CValidateHelper::username($username)) {
                $json['error'] = JText::sprintf('JLIB_DATABASE_ERROR_VALID_AZ09', 2);
                die( json_encode($json) );
            }
    ...
    public function ajaxShowTnc($fb = false)
    {
        ...
        $json = array(
            ...
            'html'  => '<span class="tnc">'.nl2br( $html ).'</span>'
        );
