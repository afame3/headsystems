...
class CommunityVideosController extends CommunityBaseController
{
    ...
    public function myvideos()
    {
        ...
        if ($this->checkVideoAccess()) {
            ...
            if (!$user->id) {
                $redirectUrl = CRoute::_('index.php?option=com_community&view=videos', false);
    ...
    public function link()
    {
        ...
        list($creatorType, $videoLimit) = $this->_manipulateParameter($pageid, $groupid, $eventid, $config);
        ...
        if (!$isValid) {
            $mainframe->redirect($redirect, JText::_('COM_COMMUNITY_VIDEOS_ADD_LINK_FAILED'), 'error');
            return;
        }
        ...
        $video->set('pageid', $pageid);
        $video->set('groupid', $groupid);
        $video->set('eventid', $eventid);
        $video->set('params', $video->get('_provider')->get('params'));
        ...
        if (!$video->isExist()) {
            ...
            $act->pageid = ($video->pageid) ? $video->pageid : 0;
        ...
        $message = str_replace('_QQQ_', '"', JText::sprintf('COM_COMMUNITY_VIDEOS_LINK_EXIST_WARNING', $video->getPermalink()));
        $mainframe->redirect($redirect, $message);
    }
    ...
    public function uploadVideo()
    {
        ...
        $pageid = ($creatorType == VIDEO_PAGE_TYPE) ? $jinput->getInt('pageid', 0) : 0;
        ...
        if ($pageid != 0 || $groupid != 0 || $eventid != 0) {
            list($creatorType, $videoLimit) = $this->_manipulateParameter($pageid, $groupid, $eventid, $config);
        } else {
        ...
        // add indicator if the video uploaded from sharebox
        if (count($jinput->post->getArray()) <= 5) {
            if (!isset($params)) $params = new CParameter();

            $params->set('sharebox', 1);
            $video->set('params', $params->toString());
        }
    ...
    public function saveVideo()
    {
        ...
        $activityModel->updatePermission($video->permissions, null, $video->creator, $app, $video->id);

        //update location in activity stream
        $data = array('app' => $app, 'cid' => $video->id);
    ...
    public function ajaxRemoveWall($wallId)
    {
        ...
        if (COwnerHelper::isCommunityAdmin() || ($my->id == $video->creator) || $my->authorise('community.delete','walls', $wall)) {
    ...
    public function ajaxEditVideo($videoId, $redirectUrl = '')
    {
        ...
        // Access check: ACL
        if (CFactory::getUser()->authorise('community.videoedit', 'com_community') || $video->creator == $my->id || $group->isAdmin($my->id)) {
            ...
            if ($video->pageid != 0 || $video->groupid != 0 || $video->eventid != 0) {
                $cWindowsHeight = 280;
                $showPrivacy = false;
            } else {
                $cWindowsHeight = 350;
    ...
    public function ajaxAddVideo($creatorType = VIDEO_USER_TYPE, $contextid = 0)
    {
        ...
        if ($creatorType != VIDEO_PAGE_TYPE && $creatorType != VIDEO_GROUP_TYPE && $creatorType != VIDEO_EVENT_TYPE) {
            $contextid = 0;
        }
      
        if ($creatorType == VIDEO_PAGE_TYPE) {
            $page = JTable::getInstance('Page', 'CTable');
            $page->load($contextid);
            $isBanned = $page->isBanned($my->id);

            if ($isBanned) {
                $json['error'] = JText::_('COM_COMMUNITY_PAGES_VIDEO_BANNED');
                die( json_encode($json) );
            }
        } else if ($creatorType == VIDEO_GROUP_TYPE) {
            $group = JTable::getInstance('Group', 'CTable');
    ...
    public function getLinkVideoHtml($creatorType = VIDEO_USER_TYPE, $contextid = 0)
    {
        ...
        $tmpl->set('creatorType', $creatorType);
        $tmpl->set('pageid', $creatorType == VIDEO_PAGE_TYPE ? $contextid : '');
    ...
    public function ajaxLinkVideoPreview($videoUrl)
    {
        ...
        if (!$isValid) {
            $objResponse->addScriptCall('__throwError', JText::_('COM_COMMUNITY_VIDEOS_ADD_LINK_FAILED'));
            $objResponse->sendResponse();
        }
        ...
        if ($video->isExist()) {
            $objResponse->addScriptCall('__throwError', JText::_('COM_COMMUNITY_VIDEOS_LINK_EXIST_MESSAGE'));
            $objResponse->sendResponse();
        }
    ...
    private function _manipulateParameter($pageid, $groupid, $eventid, $config)
    {
        if($pageid){
            $allowManageVideos = CPageHelper::allowManageVideo($pageid);
            CError::assert($allowManageVideos, '', '!empty', __FILE__, __LINE__);

            $creatorType = VIDEO_PAGE_TYPE;
            $videoLimit = $config->get('pagevideouploadlimit');
        }elseif($groupid){
    ...
    private function _getParameter($creatorType, $config)
    {
        ...
        } elseif ($creatorType == VIDEO_PAGE_TYPE) {
            $totalVideos = $model->getVideosCount($my->id, VIDEO_PAGE_TYPE);
            $videoUploadLimit = $config->get('pagevideouploadlimit');
    ...
    public function ajaxShowStreamVideoWindow($activity_id)
    {
        ...
        $providerName = CStringHelper::strtolower($headMetaParams->get('video_provider'));
    ...
    public function ajaxShowVideoWindow($video_id)
    {
        ...
        $enableFeatureThis = false;

        if (COwnerHelper::isCommunityAdmin() && !$video->groupid && !$video->eventid && $config->get('show_featured') && ($video->permissions == 0 || $video->permissions == 10)) {
            $featured = new CFeatured(FEATURED_VIDEOS);
            ...
            if (!in_array($video->id, $featuredList)) {
                $enableFeatureThis = 'add';
            } else {
                $enableFeatureThis = 'remove';
            }
        }
        ...
        $json['can_edit'] = (int) $my->authorise('community.edit', 'videos', $video);
        ...
        $json['enablefeature'] = $enableFeatureThis;
        ...
        $json['lang'] = array(
            ...
            'make_feature'         => JText::_('COM_COMMUNITY_MAKE_FEATURED'),
            'remove_featured'      => JText::_('COM_COMMUNITY_REMOVE_FEATURED')
        );
    ...
    private function getVideoInfoHeader($video)
    {
        ...
        $enableVideoLocation = (!empty($video->location) && $config->get('videosmapdefault') == 1);

        $template = new CTemplate();
        $info = $template->set('creator', $creator)
            ->set('videoTitle', $video->title)
            ->set('enableVideoLocation', $enableVideoLocation)
            ->set('videoLocation', $video->location)
        ...
        if ($enableReaction && $my->id && $config->get('enablewalllikereaction')) {
            $reactId = CLikesHelper::getReactId('videos', $video->id, $my->id);
            $action = '<div class="joms-stream__actions">';
            $action .= CLikesHelper::renderReactionButton('page', 'videos', $video->id, $reactId);
            $action .= '</div>';
            
            $like = new CLike;
            $reactHtml = $like->showWhoReacts('videos', $video->id);
            $show = $reactHtml ? '' : 'style="display:none"';
            $status = '<div class="joms-stream__status" '.$show.'>';
            $status .= $like->showWhoReacts('videos', $video->id);
            $status .= '</div>';
        }
    ...
    public function ajaxGetInfo($videoId, $showAllComments = false)
    {
        ...
        $wallContents = CWallLibrary::getWallContents(
            ...
            $showAllComments ? false : $config->get('stream_default_comments', 0),
            0,
            'wall/content',
            'videos,video'
        );
        ...
        $json['description'] = array(
            'content'          => CActivities::format($video->description),
            'rawcontent'       => $video->description,
    ...
    public function ajaxSaveDescription($videoId, $description)
    {
        ...
        if ( COwnerHelper::isCommunityAdmin() || $video->creator == $my->id || $group->isAdmin($my->id) ) {
            ...
            $video->store();
            $json['success'] = true;
            $json['caption'] = CActivities::format($video->description);
            $json['excerpt'] = JHTML::_('string.truncate', CActivities::format($video->description), $config->getInt('streamcontentlength'));
        } else {
            $json['error'] = JText::_('COM_COMMUNITY_NOT_ALLOWED_TO_ACCESS_SECTION');
        }
