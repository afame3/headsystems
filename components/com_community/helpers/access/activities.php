...
Class CActivitiesAccess implements CAccessInterface{
	...
	static public function activitiesCommentAdd($userId, $assetId, $obj=NULL){
		...
		if($obj instanceof CTableEvent || $obj instanceof CTableGroup){
			...
			// if all activity comment is allowed, return true
			$allow_comment = true;
		}else if(isset($obj->access) && COwnerHelper::isRegisteredUser() && $obj->access <= 20) {
            // if site members privacy
            $allow_comment = true;
        }
        ...
        if ($eventOrGroup) {
            if($allow_comment || COwnerHelper::isCommunityAdmin()){
                $result = true;
            }
        } else {
	...
	static public function activitiesStreamEditPost($userId,$actorId,$obj = NULL){
        ...
        // Access check: ACL
		if(( ($actorId == $userId) || CFactory::getUser()->authorise('community.postcommentedit', 'com_community') || ( $obj->target == $userId && $actorId == $userId )) && $isAllowedApp){
	...
	static public function activitiesStreamDeletetPost($userId,$actorId,$obj = NULL){
		...
		if(CFactory::getUser()->authorise('community.postcommentdelete', 'com_community') && $obj->app == 'users.featured'){
			return true;
		}
    ...
    static public function activitiesStreamIgnore($userId, $actorId, $obj = NULL){
        ...
        if($userId > 0 && $userId != $actorId && !COwnerHelper::isCommunityAdmin($actorId)){
            return true;
        }
