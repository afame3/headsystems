	...
	static public function injectTags( $content,$paramsTxt,$html=false )
	{
        ...
        if (!empty( $matches)) {
			foreach ($matches as $val) {
				$replaceWith = CStringHelper::trim($params->get($val[1], null));
				
                if (!is_null($replaceWith)) {
					//if the replacement start with 'index.php', we can CRoute it
					if(CStringHelper::strpos($replaceWith, 'index.php') === 0) {
					...
					if ($html) {
                        ...
                        if (!is_null($replaceUrl)) {
							...
							//if the replacement start with 'index.php', we can CRoute it
							if (CStringHelper::strpos($replaceUrl, 'index.php') === 0) {
                            ...
                            foreach ($languages as $item) {
                                $langFile = JPATH_SITE . '/language/' . $item->lang_code . '/' . $item->lang_code . '.com_community.ini';
                                $langContent = file_get_contents($langFile);
                                $matches = preg_grep('/"'.preg_quote($replaceWith,'/').'"/', explode("\n", $langContent)); 
                                
                                if ($matches) {
                                    foreach ($matches as $key => $text) {
                                        $langConstant = explode('=', $text);
                                        $replaceWith = $langConstant[0];
                                    }
                                }
                            }
                            
							$replaceWith = '<a href="'.$replaceUrl.'">'.JText::_($replaceWith).'</a>';
						}
					}
