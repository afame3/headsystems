...
class CActivities {
    ...
    static public function removePage($ids) {
        $activitiesModel = CFactory::getModel('activities');

        return $activitiesModel->removePageActivity($ids);
    }
    ...
    static public function add($activity, $params = '', $points = 1) {
        ...
        $pageid = !empty($activity->pageid) ? $activity->pageid : 0;
        $page_access = !empty($activity->page_access) ? $activity->page_access : 0;
    ...
    private function _getData($options) {
        ...
        if (!empty($app)) {
            ...
            if ($config->get('overrideactivityprivacy')) {
            ...
            $rows = $activities->getActivities($actor, $target, $date, $maxList, $respectPrivacy, $exclusions, $displayArchived, $actid, $groupid, $eventid, $pageid, $options);
        }
        ...
        for ($i = 0; $i < count($rows) && (count($htmlData) <= $maxList); $i++) {
            ...
            if (!$row->used && count($htmlData) <= $maxList) {
                ...
                if ($row->eventid) {
                    ...
                    $act->appTitle = $pageModel->getPageName($row->pageid);
                }
                ...
                for ($j = $i; ($j < count($rows)) && ($row->getDayDiff() == $day); $j++) {
                    ...
                    if (($row->getDayDiff() == $day) && ($row->title == $title) && ($app == $row->app) && ($cid == $row->cid) && (
                            (CStringHelper::strpos($row->title, '{/multiple}') !== FALSE) ||
                            ($row->actor == $actor)
                        )
                ...
                $act->pageid = $oRow->pageid;
                $act->page_access = $oRow->page_access;
                ...
                static $albumCollection = array();
                static $videoCollection = array();
                switch ($act->app) {
                    case 'photos':
                        // Album object
                        if(array_key_exists($act->cid, $albumCollection)){
                            $act->album = $albumCollection[$act->cid];
                            $oRow->album = $act->album;
                        }else {
                            $act->album = JTable::getInstance('Album', 'CTable');
                            $act->album->load($act->cid);
                            $albumCollection[$act->cid] = $act->album;
                            $oRow->album = $act->album;
                        }
                        break;
                    case 'videos':
                         if(array_key_exists($act->cid,$videoCollection)){
                            $act->video = $videoCollection[$act->cid];
                            $oRow->video = $act->video;
                        }else {
                            $act->video = JTable::getInstance('Video', 'CTable');
                            $act->video->load($act->cid);
                            $videoCollection[$act->cid] = $act->video;
                            $oRow->video = $act->video;
                        }
                        break;
                }
    ...
    /**
     * Return the newest stream ID in browser
     * @param  string $filter the filter name
     * @param  int $filterId either group, event of profileId, depending on filter
     * @return int newest stream id in browser
     */
    public function getNewestStreamId($filter = null, $filterId = 0, $filterValue = null) {
        $my = CFactory::getUser();
        $activitiesModel = CFactory::getModel('activities');
        ...
        $result = $activitiesModel->getActivities($actor, $friendsIds, null, 1, true, null, true, null, $groupid, $eventid, $pageid, $filters, true);
        
        // get latest stream ID at browser
    ...
    public function getLatestStreamCount($id, $filter, $filterId, $filterValue) {
        ...
        $result = $activitiesModel->countActivities($actor, $friendsIds, null, 0, true, $id * (-1), true, null, $groupid, $eventid, $pageid, $filters, true);
    ...
    public function getLatestStream($id, $filter, $filterId = 0, $filterValue) {
        ...
        $result = $activitiesModel->getActivities($actor, $friendsIds, null, 0, true, $id * (-1), true, null, $groupid, $eventid, $pageid, $filters, true);
...
#var_dump($filters);die();
        ...
        $result = $activitiesModel->getActivities($actor, $friendsIds, null, $config->get('maxactivities'), true, $id, true, null, $groupid, $eventid, $pageid, $filters);

        foreach ($result as $key => $val) {
            if (is_object($val->params)) {
                if (($val->params->get('action') == 'page.create' && $filter == 'active-page') || ($val->params->get('action') == 'group.create' && $filter == 'active-group') || ($val->params->get('action') == 'events.create' && $filter == 'active-event' )) {
                    unset($result[$key]);
                }
            }
        }
    ...
    public function getHTML($actor, $target, $date = null, $maxEntry = 0, $type = '', $idprefix = '', $showActivityContent = true, $showMoreActivity = false, $exclusions = null, $displayArchived = false, $filter = 'all', $latestId = 0, $options = array()) {
        ...
        if ($maxEntry == 0) {
            $maxList = ($type == 'profile') ? $config->get('activityLimit') : $config->get('maxactivities');
        } else {
            $maxList = $maxEntry;
        }
        ...
        // set up the filter id and value
        $filterId = 0;
        $filterValue = '';
        ...
        switch ($filter) {
            ...
            case "active-profile-and-friends" :
                ...
                break;
            case "hashtag" :
                $filterValue = $options['hashtag'];
                break;
            case "keyword" :
                $filterValue = $options['keyword'];
                break;
            case "all":
            default :
                break;
    ...
    static public function _createdLapse(&$date, $showFull = true) {
        ...
        //if ($html != JText::_('COM_COMMUNITY_LAPSED_YESTERDAY')) $html .= JText::_('COM_COMMUNITY_LAPSED_AGO');
    ...
    static public function getActivitiesByFilter($filter = 'all', $userId = 0, $view = '', $showMore = true, $filters = array(), $extra = false) {
        ...
        switch ($filter) {
            ...
            case "active-profile" :
                ...
                $actLimit = ($view == 'profile') ? $config->get('activityLimit') : $config->get('maxactivities');
            ...
            case "active-profile-and-friends" :
                $params = $user->getParams();
                $actLimit = ($view == 'profile') ? $config->get('activityLimit') : $config->get('maxactivities');
                $filter = 'active-profile-and-friends';
                if (JFactory::getUser()->id == $user->id) {
                    $filter = 'me-and-friends';
                }

                $html = $act->getHTML($user->id, $friendIds, $memberSince, $actLimit, $view, '', true, $showMore, null, false, $filter, null, array(), $extra);
                break;

            case "hashtag":
                $html = $act->getHTML('', '', null, 0, $view, '', true, $showMore, null, false, 'hashtag', 0, $filters, $extra);
                break;

            case "keyword":
                $html = $act->getHTML('', '', null, 0, $view, '', true, $showMore, null, false, 'keyword', 0, $filters, $extra);
                break;
...
static public function formatSharePopup($act) {
    switch ($act->app) {
        ...
        case 'pages':
            ...
            $act->title = $page->name;
            $act->content = new stdClass();
            $act->content->title = $page->name;
            $act->content->description = $page->description;
            $act->content->thumb = $page->getThumbAvatar();
            break;
        case 'groups':
...
static public function generateShareHtml($act) {
    ...
    if (is_object($act->content) && ($act->app == 'pages') || ($act->app == 'groups') || ($act->app == 'events')) {
...
static public function formatStreamAttachment($obj) {
    switch ($obj->app) {
        ...
        case 'pages':
            $attachment = new stdClass();
            $attachment->type = 'page_share';

            $page = JTable::getInstance('Page', 'CTable');
            $page->load($obj->cid);
            $attachment->message = new stdClass;

            $attachment->message->title = $page->name;
            $attachment->message->description = $page->description;
            $attachment->message->link = $page->getLink();
        case 'groups':
            $attachment = new stdClass();
