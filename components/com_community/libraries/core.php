...
// to prevent fatal error at jomsocial API
try {
...
class CFactory {
    ...
    public static function getUser($id = null, $obj = null) {
        ...
        if ($id === 0) {
            ...
            if(is_numeric($id) && empty(self::$instances[$id])){
                //we must make sure this user exists
                $query = 'SELECT ' . $db->quoteName('id') . ' FROM ' . $db->quoteName('#__users') . ' WHERE ' . $db->quoteName('id') . ' = ' . $db->Quote($id);
                $db->setQuery($query);
                $testUser = $db->loadResult();
    ...
    static public function getRequestUser() {
        static $usersCollection = array();

        $mainframe = JFactory::getApplication();
        $jinput = $mainframe->input;
        $id = $jinput->get('userid', '', 'INT');

        if($id == '' && array_key_exists(0, $usersCollection)){
            return $usersCollection[0];
        }elseif(!array_key_exists($id, $usersCollection)){
    ...
    static function autoload_models($classname) {
        ...
        $classes = array(
            ...
            'CommunityModelFollowers' => '/models/followers.php',
            'CommunityModelPageCategories' => '/models/pagecategories.php',
            'CommunityModelPages' => '/models/pages.php',
        );
    ...
    static function autoload_libraries($classname) {
        $classes = array(
            ...
            'CExtraNotification' => '/libraries/extra_notification.php',
            'CParameter' => '/libraries/parameter.php',
            ...
            'CFieldsLocation' => '/libraries/fields/location.php',
            ...
            'CPages' => '/libraries/pages.php',
            'CLinkedin' => '/libraries/linkedin.php',
            'CGoogle' => '/libraries/google.php'
        );
    ...
    static function autoload_helpers($classname) {
        $classes = array(
            ...
            'CInboxHelper' => '/helpers/inbox.php',
            'CServiceCommentHelper' => '/helpers/commentservice.php',
            'CFollowersHelper' => '/helpers/followers.php',
            'CPageHelper' => '/helpers/page.php',
        );
    ...
    static function autoload_tables($classname) {
        $classes = array(
            ...
            'CTableFollower' => '/tables/follower.php',
            'CTablePage' => '/tables/page.php',
            'CTablePageMembers' => '/tables/pagemembers.php',
            'CTablePageCategory' => '/tables/pagecategory.php',
            'CTablePageInvite' => '/tables/pageinvite.php',
            'CTableRating' => '/tables/rating.php'
        );
        ...
        $backendClassname = array(
            ...
            'CommunityTableGroups' => '/tables/pages.php',
            ...
            'CommunityTableRegisterInvite' => '/tables/registerinvites.php',
            'CommunityTablePageCategories' => '/tables/pagecategories.php',
        );
 ...
 * Provide global access to JomSocial configuration and paramaters
 */
class CConfig /* extends JRegistry */ {
...
class CRoute {
    ...
    static function getURI($xhtml = true) {
        // #1506 missing icon when saved privacy page
    ...
    static function _($url, $xhtml = true, $ssl = null) {
        ...
        parse_str(str_ireplace('index.php?', '', $url), $result);
        ...
        if (empty($itemid[$view])) {
            ...
            if ($currentOption == 'com_community' && $currentView == $view && $Itemid != 0) {
                ...
                //     $isValid = true;
                // } else {
                //     // using jomsocial frontpage if exist
                //     $defaultItemid = CRoute::_getDefaultItemid();
...
class CString {
    ...
    static public function _utf8_ireplace($search, $replace, $str, $count = NULL) {
        ...
        if (!is_array($search)) {
            ...
            $search = CStringHelper::strtolower($search);

            $search = preg_quote($search, '/');
            $lstr = CStringHelper::strtolower($str);
            $i = 0;
            $matched = 0;
