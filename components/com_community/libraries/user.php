...
class CUser extends JUser implements CGeolocationInterface {
        ...
        if (!$this->_init) {
            ...
            if ($initObj == null) {
                $query = 'SELECT  '
                        ...
                        . ' a.' . $db->quoteName('pages') . ' as _pages, '
                        ...
                        . ' a.' . $db->quoteName('longitude') . ' as longitude, '
                        . ' a.' . $db->quoteName('latitude') . ' as latitude, '
                        . ' a.' . $db->quoteName('default_avatar') . ' as _defaultAvatar, '
                        . ' a.' . $db->quoteName('usergroup') . ' as _usergroup, '
            ...
            if (empty($obj)) {
                if (!$obj && ($this->id != 0)) {
                    ...
                    if ($isValid) {
                        ...
                        //this is used to set the id of the person who invited this user if applicable
                        $mySess = JFactory::getSession();
                        $invitationId = $mySess->get('invitor_id', 0);

                        // Load default params
                        $obj->params = "notifyEmailSystem=" . $config->get('privacyemail') . "\n"
                                . "privacyPrivateMessage=" . $config->get('privacyprivatemessage') . "\n"
                                . "privacyFriendRequest=" . $config->get('privacyfriendrequest') . "\n"
                                ...
                                . "privacyPagesView=" . $config->get('privacy_pages_list') . "\n"
    ...
    public function isInPage($id) {
        // If id is null or empty, you're not in it of course
        if (empty($id))
            return false;

        $pages = explode(',', $this->_pages);
    ...
    /**
     * Update internal list of pages this user is a member of
     * Run only when user is logged in
     */
    public function updatePageList($forceUpdate = false) {
        $my = CFactory::getUser();

        if ($my->id == $this->id || $forceUpdate) {
            if (!class_exists('CommunityModelPages')) {
                $pageModel = CFactory::getModel('pages');
            } else {
                $pageModel = new CommunityModelPages();
            }

            $pagesIds = $pageModel->getPageIds($this->id);
            
            if ($pagesIds) {
                $pagesIds = implode(',', $pagesIds);

                if ($this->_pages != $pagesIds) {
                    $this->_pages = $pagesIds;
                    try {
                        $this->save();
                    }catch(Exception $e){
                        return false;
     ...
     */
    public function getDisplayName($rawFormat = false, $badge = false) {
        $config = CFactory::getConfig();
        ...
        $name = CStringHelper::escape($name);
        ...
        if ($badge) {
            ...
            $groups = JUserHelper::getUserGroups($this->_userid);
            
            $badge = '';

            foreach ($groups as $group) {
                $badgeName = 'profile-group-badge-'.$group;

                if (isset($settings['profile'][$badgeName])) {
                    if (!empty($settings['profile'][$badgeName])) {
                        $imagepath = COMMUNITY_PATH_ASSETS . $badgeName . '.' . $settings['profile'][$badgeName];

                        if (JFile::exists($imagepath)) {
                            $image = JURI::root() . 'components/com_community/assets/' . $badgeName . '-thumb.' . $settings['profile'][$badgeName];

                            $badge .= '<img src="' . $image . '" class="badgeNameImage" />';
                        }
                    }
                }
            }
     ...
     *
     */
    protected function _getGender($default = 'general') {
    ...
    public function isDefaultAvatar(){
        if (CStringHelper::stristr($this->_avatar, 'default.jpg') || empty($this->_avatar)) {
            return true;
        }
    ...
    public function getAvatar() {
        ...
        if (CStringHelper::stristr($this->_avatar, 'default.jpg')) {
            $this->_avatar = '';
        }
        ...
        if ($this->_storage != 'file' && !empty($this->_avatar) && CStringHelper::stristr($this->_avatar, 'images/watermarks') === false) {
        ...
        if(file_exists(COMMUNITY_PATH_ASSETS.$td_path.'.png')){
            $avatar = JUri::root().str_replace(JPATH_ROOT,'',COMMUNITY_PATH_ASSETS)."$td_path.png";
        }elseif(file_exists(COMMUNITY_PATH_ASSETS.$td_path.'.jpg')){
            $avatar = JUri::root().str_replace(JPATH_ROOT,'',COMMUNITY_PATH_ASSETS)."$td_path.jpg";
        }else{
            if (file_exists(COMMUNITY_PATH_ASSETS.'user-' . ucfirst($gender) . '.png')){
                $avatar = CUrlHelper::avatarURI($this->_avatar, 'user-' . ucfirst($gender) . '.png');
            } else {
                $avatar = CUrlHelper::avatarURI($this->_avatar, 'user-Male.png');
            }
        }

        if(CFactory::getConfig()->get('use_gravatar')){
            //if gravatar is enabled, we will try to get the picture from gravatar
            // set d=404 to make not found avatar detected at localhost
            $gravatarAvatar = "https://www.gravatar.com/avatar/".md5($this->email)."?s=200&d=404";
            $gravatarCheck = get_headers($gravatarAvatar);

            if (!preg_match("/404/", $gravatarCheck[0])) {
    ...
    public function getThumbAvatar() {
        ...
        static $thumbAvatar = array();

        if(isset($thumbAvatar[$this->id]) && $thumbAvatar[$this->id]){
            return $thumbAvatar[$this->id];
        }
        ...
        if (CStringHelper::stristr($this->_thumb, 'default_thumb.jpg')) {
            $this->_thumb = '';
        }
        ...
        // @rule: For default avatars and watermark avatars we don't want to alter the url behavior.
        // as it should be stored locally.
        if ($this->_storage != 'file' && !empty($this->_thumb) && CStringHelper::stristr($this->_thumb, 'images/watermarks') === false) {
            $storage = CStorage::getStorage($this->_storage);
            $thumbAvatar[$this->id] = $storage->getURI($this->_thumb);
            return $thumbAvatar[$this->id];
        }


        if($this->_thumb && file_exists(JPATH_ROOT.'/'.$this->_thumb)){
            $thumbAvatar[$this->id] = JUri::root().$this->_thumb;
            return $thumbAvatar[$this->id];
        }

        $gender = $this->_getGender();
        if (!strlen($gender)) $gender='undefined';
        $td_path = "default-$gender-avatar-thumb";
        $avatar = '';
        if (file_exists(COMMUNITY_PATH_ASSETS.$td_path.'.png')) {
            $thumbAvatar[$this->id] = JUri::root().str_replace(JPATH_ROOT,'',COMMUNITY_PATH_ASSETS)."$td_path.png";
        } elseif (file_exists(COMMUNITY_PATH_ASSETS.$td_path.'.jpg')) {
            $thumbAvatar[$this->id] = JUri::root().str_replace(JPATH_ROOT,'',COMMUNITY_PATH_ASSETS)."$td_path.jpg";
        } else {
            if (file_exists(COMMUNITY_PATH_ASSETS.'user-' . ucfirst($gender) . '-thumb.png')) {
                $thumbAvatar[$this->id] = CUrlHelper::avatarURI($this->_avatar, 'user-' . ucfirst($gender) . '-thumb.png');
            } else {
                $thumbAvatar[$this->id] = CUrlHelper::avatarURI($this->_avatar, 'user-Male-thumb.png');
            }
        }
        ...
        if(CFactory::getConfig()->get('use_gravatar')){
            //if gravatar is enabled, we will try to get the picture from gravatar
            // set d=404 to make not found avatar detected at localhost
            $gravatarThumb = "https://www.gravatar.com/avatar/".md5($this->email)."?s=200&d=404";
            $gravatarCheck = get_headers($gravatarThumb);
            
            if (!preg_match("/404/", $gravatarCheck[0])) {
                $thumbAvatar[$this->id] = $gravatarThumb;
            }
        }
    ...
    public function getAvatarInfo($my = 0){
        ...
        if(isset($photoCollection[$my]) && $photoCollection[$my]->albumid){
            return $photoCollection[$my];
        }elseif(isset($photoCollection[$my])){
            return false;
        }

        $photo = JTable::getInstance('Photo', 'CTable');
        $photo->load($photoId);
        $photoCollection[$my] = $photo;
     ...
     * @param   boolean $privacy    Get field value by respect privacy settings.
     */
    public function getInfo($fieldCode = null, $privacy = true) {
     ...
     * FIELD_STREET, FIELD_CITY, FIELD_STATE, FIELD_ZIPCODE, FIELD_COUNTRY, OR FIELD_LOCATION
     */
    static public function getAddress() {
        $my = CFactory::getUser();
        $addressObj = CMapsHelper::getAddress($my->id);
        $address = '';

        if (isset($addressObj->location)) {
            $location = json_decode($addressObj->location);
            if (json_last_error() === 0) {
                unset($location->lat);
                unset($location->lng);
                $address = implode((array)$location, ',');
            } else {
                $address = $addressObj->location;
            }
        } else {
            $address = implode((array)$addressObj, ',');
        }

        // remove invalid coma separator
        $address = preg_replace("/(^,|,$)/", "", str_replace(',,', '', $address));
    ...
    public function isOnline() {
        $showOnlineStatus = $this->getParams()->get('showOnlineStatus',CFactory::getConfig()->get('showOnlineStatus',0));
    ...
    public function canCreateGroups() {
        // ACL check
        if (!CFactory::getUser()->authorise('community.groupcreate', 'com_community')) {
            return false;
        }
    ...
    public function canCreateEvents() {
        ...
        // ACL check
        $config = CFactory::getConfig();
        if (!$config->get('createevents') || !CFactory::getUser()->authorise('community.eventcreate', 'com_community')) {
            return false;
        }
    ...
    public function save($group = '') {
        ...
        $obj->pages = $this->_pages;
        $obj->groups = $this->_groups;
        $obj->events = $this->_events;
        $obj->usergroup = $this->_usergroup;
