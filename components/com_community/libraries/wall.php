    ...
    if (!class_exists('CWall')) {
        class CWall
        {
            ...
            static public function getWallInputForm($uniqueId, $ajaxAddFunction, $ajaxRemoveFunc, $viewAllLink = '')
            {
                ...
                // Access check: is this user allowed to access the create new comment
            ...
            public static function canComment($appType, $uniqueId)
            {
                $my = CFactory::getUser();
                $allowed = false;
            ...
            static public function getWallContents(
                ...
                if ($limit == 0){
                    $limit = 0; // let there be no limit at all
                }

                if($appType == 'albums' || $appType == 'photos' || $appType == 'videos' || $appType != 'discussions'){
                    $order='DESC';
            ...
            static public function _getWallHTML(
                ...
                if ($wallParam->get('attached_photo_id') > 0) {
                    ...
                    if ($wallParam->get('videotitle')) {

                        $file = $wallParam->get('videothumb');

                        // Site origin or thumbnail file
                        if (CStringHelper::substr($file, 0, 4)=='http') {
                            ...
                            if (empty($file) || !JFile::exists(JPATH_ROOT.'/'.$file)) {
                                ...
                                $videothumb = $asset->url;
                            } else {
                                $baseUrl = JURI::root();
                                $videothumb = str_replace('\\', '/', rtrim($baseUrl, '/') . '/' . ltrim($file, '/'));
                            }
                        }
                        
                        $paramsHTML .= '<div class="joms-media--video joms-js--video"';
                        $paramsHTML .= ' data-type="' . $wallParam->get('videotype') . '"';
                        $paramsHTML .= ' data-id="' . $wallParam->get('videoid') . '"';
                        $paramsHTML .= ' data-path="' . ( $wallParam->get('videotype') === 'file' ? JURI::root(true) . '/' : '' ) . $wallParam->get('videourl') . '"';
                        $paramsHTML .= ' data-width="' . $wallParam->get('width', 16) . '"';
                        $paramsHTML .= ' data-height="' . $wallParam->get('height', 9) . '"';
                        $paramsHTML .= ' style="margin-top:10px;">';
                        $paramsHTML .= '<div class="joms-media__thumbnail">';
                        $paramsHTML .= '<img src="' . $videothumb . '">';
                        ...
                        $paramsHTML .= '<h4 class="joms-media__title">' . JHTML::_('string.truncate', $wallParam->get('videotitle'), 50, true, false) . '</h4>';
                        $paramsHTML .= '<p class="joms-media__desc">' . JHTML::_('string.truncate', $wallParam->get('videodescription'), $config->getInt('streamcontentlength'), true, false) . '</p>';
                        $paramsHTML .= '</div>';
                        $paramsHTML .= '</div>';

                    } elseif ($wallParam->get('title')) {

                        $video = self::detectVideo($wallParam->get('url'));
                        $url = $wallParam->get('url') ? $wallParam->get('url') : '#';

                        if($config->get('enable_embedly') && $url != '#'){
                            $paramsHTML .="<a href=\"".$url."\" class=\"embedly-card\" data-card-controls=\"0\" data-card-recommend=\"0\" data-card-theme=\"".$config->get('enable_embedly_card_template')."\" data-card-align=\"".$config->get('enable_embedly_card_position')."\">".JText::_('COM_COMMUNITY_EMBEDLY_LOADING')."</a>";
                        }elseif (is_object($video)) {

                            $paramsHTML .= '<div class="joms-media--video joms-js--video"';
                            $paramsHTML .= ' data-type="' . $video->type . '"';
                            $paramsHTML .= ' data-id="' . $video->video_id . '"';
                ...
                $wall->comment = str_replace('&lt;br /&gt;', '<br />', $wall->comment);
                ...
                //for third party comments, only owner can edit
                if (strpos($appType, 'service.comment.') === 0) {
                    // Access check: ACL
                ...
                $reactedId = $like->getReactedId('comment', $wall->id, $my->id);
                // Create new instance of the template
                $tmpl = new CTemplate();
                return $tmpl->set('id', $wall->id)
                    ->set('author', $user->getDisplayName(false, true))
            ...
            static public function getViewAllLinkHTML($link, $count = null)
            {
                ...
                return $tmpl->set('viewAllLink', $link)
                    ->set('count', $count)
                    ->set('commentDiff', intval($count) - $config->get('stream_default_comments', 0))
                    ->fetch('wall/misc');
            }
            ...
            static public function _callFunction($controller, $method, $arguments)
            {
                ...
                require_once(JPATH_ROOT . '/components/com_community/controllers' . '/' . CStringHelper::strtolower(
                        $controller
                    ) . '.php');
            ...
            public static function formatComment($wall)
            {
                ...
                $wall->stream_id = $activity->id;

                $ownPost = ($my->id == $wall->post_by);
                $allowRemove = $my->authorise('community.delete','walls', $wall);

                // Access check: ACL
                if (!$my->authorise('community.postcommentcreate', 'com_community')) {
                    $canEdit = false;
                } else {
                    // Access check: ACL
                    $canEdit = ($config->get('wallediting') && $my->id == $wall->post_by) || $my->authorise('community.postcommentedit', 'com_community');
                }
                ...
                if ($params->get('attached_photo_id') > 0) {
                    ...
                    if ($params->get('videotitle')) {

                        $file = $params->get('videothumb');
                        ...
                        $paramsHTML .= '<div class="joms-media--video joms-js--video"';
                        $paramsHTML .= ' data-type="' . $params->get('videotype') . '"';
                        $paramsHTML .= ' data-id="' . $params->get('videoid') . '"';
                        $paramsHTML .= ' data-path="' . ( $params->get('videotype') === 'file' ? JURI::root(true) . '/' : '' ) . $params->get('videourl') . '"';
                        $paramsHTML .= ' data-width="' . $params->get('width', 16) . '"';
                        $paramsHTML .= ' data-height="' . $params->get('height', 9) . '"';
                        $paramsHTML .= ' style="margin-top:10px;">';
                        $paramsHTML .= '<div class="joms-media__thumbnail">';
                        $paramsHTML .= '<img src="' . $videothumb . '">';
                        $paramsHTML .= '<a href="javascript:" class="mejs-overlay mejs-layer mejs-overlay-play joms-js--video-play joms-js--video-play-' . $wall->id . '">';
                        ...
                        $paramsHTML .= '</div>';
                        $paramsHTML .= '<div class="joms-media__body">';
                        $paramsHTML .= '<h4 class="joms-media__title">' . JHTML::_('string.truncate', $params->get('videotitle'), 50, true, false) . '</h4>';
                        $paramsHTML .= '<p class="joms-media__desc">' . JHTML::_('string.truncate', $params->get('videodescription'), $config->getInt('streamcontentlength'), true, false) . '</p>';
                        $paramsHTML .= '</div>';
                        $paramsHTML .= '</div>';

                    } elseif ($params->get('title')) {
                ...
                $template
                    ...
                    ->set('activityId', $params->get('activityId'))
                    ->set('date', $date)
                    ->set('isLiked', $isLiked)
                    ->set('likeCount', $likeCount)
                    ->set('canEdit', $canEdit)
                ...
                if(!!$config->get('enablereaction')) {
                    $reactedId = $like->getReactedId('comment', $wall->id, $my->id);
                    $template->set('reactedId', $reactedId);
                }
