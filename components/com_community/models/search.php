...
//		$total		= $db->loadResult();
	...
	*/
	...
	public function searchVideo( $searchText )
	{
        ...
        $where = array();
        $where[] = 'v.' . $db->quoteName('published') . ' = ' . $db->quote(1);
        $where[] = 'v.' . $db->quoteName('status') . ' = ' . $db->quote('ready');
        $where[] = '( v.' . $db->quoteName('permissions') . ' <= ' . $db->quote(20) . '  OR ( v.creator=' . $db->Quote($my->id) . ' AND v.permissions <=' . $db->Quote(40) . ' ) )';
        ...
        $where  .= ' AND (v.' . $db->quoteName( 'title' ) . ' LIKE ' . $db->Quote( '%' . $searchText . '%' ) . ' OR v.' . $db->quoteName( 'description' ) . ' LIKE ' . $db->Quote( '%' . $searchText . '%' ) . ') ';
		...
		// echo $query	= 'SELECT *, ' . $db->quoteName('created') . ' AS lastupdated '
