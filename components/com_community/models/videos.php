...
class CommunityModelVideos extends JCCModel implements CLimitsInterface {
    ...
    public function getVideos($filters = array(), $tableBind = false, $limitSetting = true, $useCache = true, $pagination = true) {
            ...
            if ($value || $value === 0) {
                switch (strtolower($field)) {
                        ...
                        if (isset($filters['friendsvideos']) && $filters['friendsvideos']) {
                            ...
                            if ($my->id != 0) {
                                if (empty($filters['groupid'])) {
                                    ...
                                    if (!empty($friends)) {
                                        $extrasql .= ' OR (v.creator IN(' . implode(',', $friends) . ') AND v.permissions = ' . $db->Quote(30) . ') ';
                                    }
                                }
                            }
                            $where[] = '( v.' . $db->quoteName('permissions') . ' <= ' . $db->quote((int) $value) . ' ' . $extrasql . '  OR ( v.creator=' . $db->Quote($my->id) . ' AND v.permissions <=' . $db->Quote(40) . ' ) )';
                        } else {
                    ...
                    case 'status':
                        ...
                        break;
                    case 'pageid':
                        $where[] = 'v.' . $db->quoteName('pageid') . ' = ' . $db->quote($value);
                        break;
                    case 'groupid':
                    ...
                    case 'eventid':
                        ...
                        break;
                    case 'pagesonly':
                        $where[] = 'v.' . $db->quoteName('pageid') . ' > ' . $db->quote(0);
                        break;
                    case 'groupsonly':
                        $where[] = 'v.' . $db->quoteName('groupid') . ' > ' . $db->quote(0);
                        break;
                    case 'eventsonly':
                        $where[] = 'v.' . $db->quoteName('eventid') . ' > ' . $db->quote(0);
                        break;
                    case 'limitstart':
        ...
        if ($my->id > 0 && !COwnerHelper::isCommunityAdmin()) {
            ...
            if ($blockedUserIds) {
                $where .= ' AND v.'.$db->quoteName('creator').' NOT IN (' . $blockedUserIds . ') ';
            }
        }

        // Joint with group table
        $join = '';
        if (isset($filters['or_group_privacy']) && !COwnerHelper::isCommunityAdmin()) {
            ...
            $join .= ' LEFT JOIN ' . $db->quoteName('#__community_groups_members') . ' AS gm';
            $join .= ' ON gm.' . $db->quoteName('groupid') . ' = g.' . $db->quoteName('id');
            
            $where .= ' AND ((g.' . $db->quoteName('approvals') . ' = ' . $db->Quote('0') . ' OR g.' . $db->quoteName('approvals') . ' IS NULL)';
            $where .= ' OR (g.' . $db->quoteName('approvals') . ' = ' . $db->Quote('1') . ' AND (g.' . $db->quoteName('ownerid') . '=' . $db->Quote($my->id) . ' OR (gm.' . $db->quoteName('memberid') . '=' . $db->Quote($my->id) . ' AND gm.' . $db->quoteName('approved') . '=' . $db->Quote('1') . '))))';
        }

        if (isset($filters['or_page_privacy']) && !COwnerHelper::isCommunityAdmin()) {
            $approvals = (int) $filters['or_page_privacy'];
            $join .= ' LEFT JOIN ' . $db->quoteName('#__community_pages') . ' AS p';
            $join .= ' ON p.' . $db->quoteName('id') . ' = v.' . $db->quoteName('pageid');
            $join .= ' LEFT JOIN ' . $db->quoteName('#__community_pages_members') . ' AS pm';
            $join .= ' ON pm.' . $db->quoteName('pageid') . ' = p.' . $db->quoteName('id');
            
            $where .= ' AND ((p.' . $db->quoteName('approvals') . ' = ' . $db->Quote('0') . ' OR p.' . $db->quoteName('approvals') . ' IS NULL)';
            $where .= ' OR (p.' . $db->quoteName('approvals') . ' = ' . $db->Quote('1') . ' AND (p.' . $db->quoteName('ownerid') . '=' . $db->Quote($my->id) . ' OR (pm.' . $db->quoteName('memberid') . '=' . $db->Quote($my->id) . ' AND pm.' . $db->quoteName('approved') . '=' . $db->Quote('1') . '))))';
        }
        
        $config = CFactory::getConfig();
        if (!$config->get('enablepages')) {
            $where .= ' AND v.' . $db->quoteName('creator_type') . '!=' . $db->Quote(VIDEO_PAGE_TYPE);
        }

        if (!$config->get('enablegroups')) {
            $where .= ' AND v.' . $db->quoteName('creator_type') . '!=' . $db->Quote(VIDEO_GROUP_TYPE);
        }

        if (!$config->get('enableevents')) {
            $where .= ' AND v.' . $db->quoteName('creator_type') . '!=' . $db->Quote(VIDEO_EVENT_TYPE);
        }
    ...
    public function getPageVideos($pageid, $categoryid = "", $limit = "", $sorting = "") {
        $mainframe = JFactory::getApplication();
        $jinput = $mainframe->input;
        $filter = array(
            'pageid' => $pageid,
    ...
    public function getInviteListByName($namePrefix, $userid, $cid, $limitstart = 0, $limit = 8) {
        ...
        if ($my->id) {
            if ($namePrefix === '') {
                ...
                $found = CStringHelper::strpos($my->getDisplayName(), $namePrefix);
            }
