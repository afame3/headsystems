...
function CommunityBuildRoute(&$query)
{
	...
	{
		$userTasks = array('myvideos', 'myphotos', 'myevents', 'mygroups', 'mypages');
	...
	if (isset($query['task']))
	{
		switch ($query['task'])
		{
			case 'viewpage':
				...
				{
					$pageName = JFilterOutput::stringURLUnicodeSlug($pageName);
				}
				else
				{
					$pageName = JFilterOutput::stringURLSafe($pageName);
				}

				$segments[] = $pageid . '-' . $pageName;
			...
			break;
			default:
				if ( ! in_array($query['task'], array('myphotos', 'mygroups', 'mypages', 'myevents', 'myvideos', 'invites')))
				{
					$segments[] = $query['task'];
				}
			break;
...
function CommunityParseRoute($segments)
{
	...
	if ( ! empty($count))
	{
		...
		if ( ! empty($alias))
		{
			...
			//check for alternative alias that might exists
			if(isset($segments[2]) && !$userid && $segments[1] == 'profile' && $segments[0] == 'community'){
				// the user might exists at third slot of the segment
				$alias = $segments[2];
			}
			...
			if ( ! $userid)
			{
				...
				if (!$userid){
					if(isset($segments[1]) && $segments[1] == 'profile'){
                        $site->enqueueMessage(JText::_('COM_COMMUNITY_USER_NOT_FOUND'), 'error');
						$site->redirect(CRoute::_('index.php?option=com_community&view=frontpage', false));
					}
				}
			}
		}
	...
	if ( ! isset($selectedMenu))
	{
		if ($count > 0)
		{
			...
			if ( ! empty($segments[2]) && $segments[1] == 'viewpage')
			{
				$pageTitle      = $segments[2];
				$vars['pageid'] = _parsePage( $pageTitle );
			}
		}
	...
	if (isset($vars['userid']) && ! isset($vars['task']))
	{
		switch ($vars['view'])
		{
			case 'photos' :
	...
	if (isset($vars['task']) && $vars['task'] == 'my:pages')
	{
		$vars['view'] = 'pages';
		$vars['task'] = 'mypages';
	}
	...
	if (isset($vars['task']) && $vars['task'] == 'viewpage')
	{
		$pageTitle      = $segments[count($segments) - 1];
		$vars['pageid'] = _parsePage($pageTitle);
	}
...
function CommunityGetUserId($alias)
{
	...
	if (empty($id))
	{
		...
		$query = 'SELECT '.$db->quoteName('userid')
					.' FROM '.$db->quoteName('#__community_users').' WHERE '.$db->quoteName('alias').'='.$db->Quote( $alias );
		$db->setQuery($query);
		$id = $db->loadResult();
	}
