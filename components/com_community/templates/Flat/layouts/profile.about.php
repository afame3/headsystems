                    ...
                    if (CPrivacy::isAccessAllowed($my->id, $profile['id'], 'custom', $item['access'])) {
                        ...
                        if (!empty($fieldData)) {
                            if ($item['type'] == 'location') {
                                $location = CMapsHelper::getLocationData($item['value']);
                                $fieldData = $location->name. ((!empty($location->desc)) ? ' ('.$location->desc.')' : '');
                            }
                            ?>
                            <li>
...
<div class="joms-module__footer">
    <?php if ($isMine && !$hideButton){ ?>
        <a class="joms-button--primary" href="<?php echo CRoute::_('index.php?option=com_community&view=profile&task=edit'); ?>" title="<?php echo JText::_('COM_COMMUNITY_PROFILE_EDIT'); ?>"><?php echo JText::_('COM_COMMUNITY_PROFILE_EDIT'); ?></a>
    <?php }elseif(COwnerHelper::isCommunityAdmin()){
        //admin can actually edit others' profile
        ?>
        <a class="joms-button--primary" href="<?php echo CRoute::_('index.php?option=com_community&view=profile&task=edit&userid='.$profile['id']); ?>" title="<?php echo JText::_('COM_COMMUNITY_PROFILE_EDIT'); ?>"><?php echo JText::_('COM_COMMUNITY_PROFILE_EDIT'); ?></a>
    <?php } ?>
</div>
