					...
					if(cond == "between"){
						if(type=='birthdate' || type=='date'){
							datepickerOptions = {
								...
								changeYear: true,
								showOn: "button",
								buttonImage: "<?php echo JURI::root();?>components/com_community/assets/calendar.png",
	...
	<?php }?>
		...
		if(searchHistory != ''){
			...
			joms.jQuery(keylist).each(function(){
				...
				}else if( searchHistory['fieldType'+num] == "birthdate" && ( joms.jQuery.isNumeric(searchHistory['value'+this]) || joms.jQuery.isNumeric(searchHistory['value'+this+'_2']) )){
                    jsAdvanceSearch.action.toggleAgeSearch(num,1);
                }
