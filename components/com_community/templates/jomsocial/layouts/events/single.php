...
$isFeatured = false;
if (COwnerHelper::isCommunityAdmin() && $config->get('show_featured') && !$event->unlisted) {
...
<div class="joms-body">
    ...
    <div class="joms-focus">
            ...
            <div class="joms-focus__header">
                ...
                <div class="joms-focus__actions__wrapper">
                    ...
                    <div class="joms-focus__actions--desktop">
                        ...
                        <?php if ($handler->isAllowed() && !$isPastEvent && CEventHelper::showAttendButton($event)) { ?>
                            <a href="javascript:" class="joms-focus__button--add"
                                ...
                                <?php } elseif ($event->getMemberStatus($my->id) >= COMMUNITY_EVENT_STATUS_MAYBE) { ?>
                                <span class="joms-icon__maybe-attending"></span>
                                <?php echo JText::_('COM_COMMUNITY_EVENTS_RSVP_MAYBE_ATTEND'); ?>
                                <?php } elseif ($event->getMemberStatus($my->id) >= COMMUNITY_EVENT_STATUS_WONTATTEND) { ?>
                    ...
                    <?php } else { ?>
                        <div class="joms-focus__actions--desktop">
                            <?php if ($showRequestInvitationButton) { ?>
                                <a href="javascript:" class="joms-focus__button--message" onclick="joms.api.eventJoin('<?php echo $event->id; ?>');"><?php echo JText::_('COM_COMMUNITY_LOGIN'); ?></a>
                            <?php } ?>
                        </div>
                    <?php } ?>
        ...
        <div class="joms-focus__actions">
            ...
            <ul class="joms-dropdown">
                ...
                <?php if ($memberStatus != COMMUNITY_EVENT_STATUS_BLOCKED) { ?>
                    ...
                    <li class="divider"></li>
                <?php } ?>
                <?php if ($isMine || CFactory::getUser()->authorise('community.eventedit', 'com_community') || $isAdmin) {?>
                ...
                <?php if ($isMine || $isCommunityAdmin || $isAdmin || $handler->manageable() || CFactory::getUser()->authorise('community.eventedit', 'com_community') || CFactory::getUser()->authorise('community.groupeditstate', 'com_community') || $my->authorise('community.delete', 'events.' . $event->id, $event)) { ?>
                    <?php if ($isMine || CFactory::getUser()->authorise('community.eventedit', 'com_community') || $isAdmin) {?>
                    ...
                    <?php // ACL check
                    if (CFactory::getUser()->authorise('community.eventditstate', 'com_community')) { ?>
                        <li>
                            <a href="javascript:" onclick="joms.api.eventUnpublish('<?php echo $event->id; ?>');"><?php echo JText::_('COM_COMMUNITY_EVENTS_UNPUBLISH'); ?></a>
                        </li>
                    <?php } ?>
        ...
        <ul class="joms-focus__link">
            ...
            <?php } else if (isset($pageEventDetails) && !empty($pageEventDetails->pageName)) { ?>
            <li class="full"><?php echo JText::sprintf('COM_COMMUNITY_PAGE_EVENT_HOSTED_BY', $pageEventDetails->pageLink, $pageEventDetails->pageName, CRoute::_('index.php?option=com_community&view=profile&userid=' . $pageEventDetails->creator->id), $pageEventDetails->creator->getDisplayName()) ?></li>
            <?php } else { ?>
    ...
    <div class="joms-sidebar">
        ...
        <div class="joms-module__wrapper">
            <div class="joms-tab__bar">
                <a href="#joms-event--attend" class="active"><?php echo JText::_('COM_COMMUNITY_EVENTS_RSVP_ATTEND_LIST'); ?></a>
                <?php if ($maybeCount > 0) { ?>
                    ...
                    <div id="community-event-map" >
                        <div class="app-box-content event-description">
                            ...
                            if (CFactory::getConfig()->get('maps_api', '') == 'openstreetmap') {
                                echo CMapping::drawOSMMap($event->location, '100%', '100%');
                                ?>
                                <div id="event-map" style="height:0px;">
                        ...
                        <div class="joms-module__footer">
                            ...
                            <?php } else { ?>
                                <a href="https://www.openstreetmap.org/?mlat=<?php echo $event->latitude ?>&mlon=<?php echo $event->longitude ?>#map=9/<?php echo $event->latitude ?>/<?php echo $event->longitude ?>" target="_blank" class="joms-button--link"><?php echo JText::_('COM_COMMUNITY_EVENTS_FULL_MAP'); ?></a>
                            <?php } ?>
                        </div>
                    </div>
                    <?php if (CFactory::getConfig()->get('maps_api', '') == 'googlemap') { ?>
            ...
            <?php if ($showPhotos) { ?>
                ...
                <div id="joms-event--photos" class="joms-tab__content">
                    ...
                    <div class="cUpdatesHelper clearfull">
                        <a href="<?php echo CRoute::_(
                            'index.php?option=com_community&view=photos&eventid=' . $event->id
                                 ); ?>">
        ...
        <div id="events-invite-<?php echo $event->id; ?>" class="joms-page joms-js--invitation-notice-event-<?php echo $event->id; ?>">
            <div class="cInvite-Content">
                <div class="cInvite-Message">
                    <?php echo JText::sprintf('COM_COMMUNITY_EVENTS_YOUR_INVITED', $join);
                    $test = 1; ?>
                </div>
                <?php if ($friendsCount) { ?>
                ...
                <div class="cInvite-Actions">
                    <a href="javascript:"
                        onclick="joms.api.eventResponse('<?php echo $event->id; ?>',
                            ...
                            ['<?php echo COMMUNITY_EVENT_STATUS_MAYBE; ?>', '<?php echo JText::_('COM_COMMUNITY_EVENTS_RSVP_MAYBE_ATTEND', true); ?>'],
                            ['<?php echo COMMUNITY_EVENT_STATUS_WONTATTEND; ?>', '<?php echo JText::_('COM_COMMUNITY_EVENTS_RSVP_NOT_ATTEND', true); ?>']);">
                        <?php echo JText::_('COM_COMMUNITY_EVENTS_RSVP_NOTIFICATION'); ?>
                    </a>
