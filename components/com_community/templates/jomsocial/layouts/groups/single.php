...
$aclaccess = CFactory::getUser()->authorise('community.groupeditstate', 'com_community') || CFactory::getUser()->authorise('community.groupedit', 'com_community') || CFactory::getUser()->authorise('community.groupdelete', 'com_community');
...
$isFeatured = false;
if ($isSuperAdmin && $config->get('show_featured') && !$group->unlisted) {
...
<div class="joms-focus__cover">
    ...
    <div class="joms-focus__header">
        <div class="joms-avatar--focus">
            ...
            <?php } ?>>
                ...
                <?php if ($isAdmin || CFactory::getUser()->authorise('community.groupedit', 'com_community') || $isMine) { ?>
            ...
            <div class="joms-focus__actions--desktop" style="position:relative">
                ...
                <?php } elseif(!$isBanned) { ?>
                    ...
                    <a href="javascript:" class="joms-focus__button--message" onclick="joms.api.groupJoin('<?php echo $group->id; ?>')">
                        <?php
                            if ($my->id) {
                                echo JText::_('COM_COMMUNITY_GROUPS_JOIN');
                            } else {
                                echo JText::_('COM_COMMUNITY_LOGIN');
                            }
                        ?>
                    </a>
                <?php } ?>
    ...
    <?php if ($isMember && !$isBanned || $isSuperAdmin || (CFactory::getUser()->authorise('community.groupedit', 'com_community') || CFactory::getUser()->authorise('community.groupeditstate', 'com_community') || CFactory::getUser()->authorise('community.groupdelete', 'com_community'))) { ?>
...
<div class="joms-focus__actions" style="position:relative;">
    ...
    <?php if (
        ...
        $isSuperAdmin ||
        (CFactory::getUser()->authorise('community.groupedit', 'com_community') || CFactory::getUser()->authorise('community.groupeditstate', 'com_community') || CFactory::getUser()->authorise('community.groupdelete', 'com_community'))
    ) { ?>
    ...
    <ul class="joms-dropdown">
        ...
        if ( ($isMember && !$isBanned && !$waitingApproval) || $isSuperAdmin || (CFactory::getUser()->authorise('community.groupedit', 'com_community') || CFactory::getUser()->authorise('community.groupeditstate', 'com_community') || $my->authorise('community.delete', 'groups.' . $group->id, $group))) { ?>
            ...
            if ($isAdmin || $isSuperAdmin || $isMine || CFactory::getUser()->authorise('community.groupedit', 'com_community')) { ?>
            ...
            if ($isMine || $isSuperAdmin || $isAdmin || (CFactory::getUser()->authorise('community.groupedit', 'com_community') || CFactory::getUser()->authorise('community.groupeditstate', 'com_community') || $my->authorise('community.delete', 'groups.' . $group->id, $group))) { ?>

                <?php if ($isMine || $isAdmin || CFactory::getUser()->authorise('community.groupedit', 'com_community')) { ?>
                <li>
                ...
                <?php if (($isMine || $isSuperAdmin || $isAdmin) && $config->get('createannouncement',0)) { ?>
                    <li>
...
if($showGroupDetails || $aclaccess){
    ...
    <ul class="joms-focus__link">
        ...
        <?php if ($showPhotos) { ?>
            <li class="half">
                <a href="<?php echo CRoute::_('index.php?option=com_community&view=photos&groupid=' . $group->id); ?>"><?php echo ($totalPhotos == 1) ?
        ...
        <?php if (((($isAdmin) || ($isMine) || ($isMember && !$isBanned)) && $isFile) || ($isAdmin || $isSuperAdmin)){ ?>
            <li class="full">
                <a href="javascript:" data-ui-object="joms-dropdown-button">
    ...
    <div class="joms-module__wrapper">
        ...
        <?php if($allowDiscussion) { ?>
            <div id="joms-group--discussion" class="joms-tab__content">
                <?php
                if ($group->approvals == '0' || $isMine || ($isMember && !$isBanned) || $isSuperAdmin || $aclaccess) {
                    if ($config->get('creatediscussion')) {
                                        ...
                                        <span class="joms-stream__time">
                                            <small>
                                                <?php echo JText::sprintf('COM_COMMUNITY_GROUPS_DISCUSSION_CREATOR',
                                                    '<a href="' . CUrlHelper::userLink($row->user->id) . '">' . $row->user->getDisplayName(false, true) . '</a>'); ?>
                        ...
                        <div class="cUpdatesHelper clearfull">
                          <span class="updates-options cFloat-R">
                            <?php if ((($isMember && !$isBanned) && !($waitingApproval) || $isSuperAdmin || $aclaccess) && $my->authorise('community.create', 'groups.discussions.' . $group->id)): ?>
                                            ...
                                            <span class="joms-comment__time">
                                                <small>
                                                    ...
                                                    <?php echo JText::sprintf('COM_COMMUNITY_BULLETIN_CREATED_BY',
                                                        $row->creator->getDisplayName(false, true),
...
<!-- Group's Module' -->
<?php if ($group->approvals == '0' || $isMine || ($isMember && !$isBanned) || $isSuperAdmin || $aclaccess) { ?>
    <div class="joms-module__wrapper">
        ...
        <?php if ($showEvents) { ?>
            <div id="joms-group--events" class="joms-tab__content" style="display:none;">
                ...
                <div class="cUpdatesHelper clearfull">
                    <a href="<?php echo CRoute::_(
                        'index.php?option=com_community&view=events&groupid=' . $group->id
                    ); ?>">
    ...
    <div class="joms-module__wrapper">
        ...
        <?php if ($albums) { ?>
            <div id="joms-group--photos" class="joms-tab__content">
                ...
                <div class="cUpdatesHelper clearfull">
                    <a href="<?php echo CRoute::_(
                        'index.php?option=com_community&view=photos&groupid=' . $group->id
                    ); ?>">
        ...
        <?php if ($videos) { ?>
            <div id="joms-group--videos" class="joms-tab__content" style="display:none;" >
                ...
                <div class="cUpdatesHelper clearfull">
                    <a href="<?php echo CRoute::_(
                        'index.php?option=com_community&view=videos&groupid=' . $group->id
                    ); ?>">
...
<div class="joms-main">
    ...
    <div class="joms-middlezone">
        ...
        <div class="joms-tab__bar">
            ...
            <?php } ?>
            <a href="#joms-group--details" class="<?php echo (($showGroupDetails && $config->get('default_group_tab') == 1) || (!$showGroupDetails) && !$aclpost) ? 'active' : ''; ?>">
        ...
        if($showGroupDetails || $aclpost){
            ...
            <div id="joms-group--stream" class="joms-tab__content" style="<?php echo (!$showGroupDetails || $config->get('default_group_tab') == 0) ? '' : 'display:none'; ?>">
                <?php if($isMember || $isSuperAdmin || $isAdmin || (!$config->get('lockgroupwalls', false) && !$group->approvals)) {$status->render();} ?>
                <?php echo $streamHTML; ?>
            </div>
        <?php } ?>
        <div id="joms-group--details" class="joms-tab__content" style="<?php echo (($showGroupDetails && $config->get('default_group_tab') == 1) || (!$showGroupDetails && !$aclpost)) ? '' : 'display:none'; ?>">
                ...
                <li>
                    <h5 class="joms-text--light"><?php echo JText::_('COM_COMMUNITY_GROUPS_CATEGORY'); ?></h5>
                        <span><a href="<?php echo CRoute::_(
                                'index.php?option=com_community&view=groups&categoryid=' . $group->categoryid
