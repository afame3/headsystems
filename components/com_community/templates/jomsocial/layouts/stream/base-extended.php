...
if (!isset($stream->pageid)) {
    $stream->pageid = '';
}

if (!isset($stream->eventid)) {
    $stream->eventid = '';
}
...
<?php endif; ?>
    ...
    <div class="joms-stream__meta">
        <?php
        if ($stream->actor->id == 0) {
            $actorLink = $stream->actor->getDisplayName(false, true);
        } else {
            $actorLink = '<a href="' . CUrlHelper::userLink($stream->actor->id) . '">' . $stream->actor->getDisplayName(false, true) . '</a>';
        }

        echo CString::str_ireplace('{actor}', $actorLink, $stream->headline);
        ?>
        <a href="<?php echo ((isset($streamURL)) ? $streamURL : ''); ?>">
...
<?php
$disableStreamContent = false;

if (!empty($stream->attachments)) {
    foreach ($stream->attachments as $attachment) {
        if ($attachment->type == 'text') {
            $disableStreamContent = true;
        }
    }
}
?>

<?php if (isset($stream->message) && !$disableStreamContent) {?>
    <?php if (isset($stream->sharedMessage)) {?>
    <div data-type="stream-editor" class="cStream-Respond" style="display:none">
        <textarea class="joms-textarea" style="margin-bottom:0"><?php echo $stream->rawmessage; ?></textarea>
        ...
        switch ($attachment->type) {
            ...
            case 'video':
                ...
                if ( isset($video) ) {
                    ...
                    $videoParams = new JRegistry($video->params);
                ...
                <div class="joms-media--video joms-js--video"
                    ...
                    <div class="joms-media__body <?php echo ($config->getInt('enablevideosthumbbigsize') ? 'fullwidth-thumb-video' : ''); ?>">
                ...
                if (strlen($attachment->message)):
                    ...
                    <div class="cStream-Attachment">
                        ...
                        <div class="joms-quote joms-text--desc">
                            <?php
                            $background = null;
                            if ($stream->backgroundid) {
                                $backgroundsModel = CFactory::getModel('Backgrounds');
                                $background = $backgroundsModel->getBackground($stream->backgroundid);
                            }
                            
                            if ($background && $config->get('enablebackground')) { ?>
            ...
            case 'text':
                $actId = $attachment->activity->get('id');
                $title = $attachment->message;

                $originalTitle = $title;
                if (isset($attachment->activity) && isset($attachment->activity->title)) {
                    $originalTitle = $attachment->activity->title;
                }

                ?>

                <div data-type="stream-editor" class="cStream-Respond" style="display:none">
                    <textarea class="joms-textarea" style="margin-bottom:0"><?php echo $originalTitle; ?></textarea>
                    <div style="text-align:right;">
                        <button class="joms-button--neutral joms-button--small" onclick="joms.view.stream.cancel('<?php echo $actId; ?>');"><?php echo JText::_('COM_COMMUNITY_CANCEL'); ?></button>&nbsp;
                        <button class="joms-button--primary joms-button--small" onclick="joms.view.stream.save('<?php echo $actId; ?>', this);"><?php echo JText::_('COM_COMMUNITY_SAVE'); ?></button>
                    </div>
                </div>
                <p data-type="stream-content">
                    <?php echo CActivities::shorten($title, $actId, 0, $config->getInt('streamcontentlength')); ?>
                    <?php if ($attachment->address) { ?>
                        <span class="joms-status-location"> -
                            <a href="javascript:" onclick="joms.api.locationView('<?php echo $actId; ?>');">
            ...
            case 'fetched':
                ...
                echo CActivities::format($stream->sharedMessage, $stream->sharedMood);
