    ...
    <ul class="joms-list--video" >
        ...
        foreach ($videos as $video) {
            ...
            <li class="joms-list__item video-permission-<?php echo $video->permissions; ?>">
                ...
                <?php if ($isCommunityAdmin || ($video->isOwner() && CFactory::getUser()->authorise('community.videocreate', 'com_community') && !$groupVideo) || ($groupVideo && $allowManageVideos) || $my->authorise('community.delete', 'videos', $video) || $my->authorise('community.edit', 'videos', $video)) { ?>
                ...
                <?php } else { ?>
                    ...
                    <div class="joms-block joms-text--light">
                        ...
                        <?php } ?>
                        <span><?php echo '<svg viewBox="0 0 18 18" class="joms-icon"><use xlink:href="' . CRoute::getURI() .'#joms-icon-eye"></use></svg> ' . $video->getHits(); ?></span> ·
                        <span><?php echo '<svg viewBox="0 0 18 18" class="joms-icon"><use xlink:href="' . CRoute::getURI() .'#joms-icon-bubble"></use></svg> ' . $video->getWallCount(); ?></span> · 
                        <span><?php echo '<svg viewBox="0 0 18 18" class="joms-icon"><use xlink:href="' . CRoute::getURI() .'#joms-icon-thumbs-up"></use></svg> ' . $likeCount; ?></span> · 
                ...
                <small>
                    <?php echo JTEXT::_('COM_COMMUNITY_VIDEOS_BY') ?>
                    ...
                    <?php if($video->pageid){
                        ...
                        $page->load($video->pageid);
                        echo JText::sprintf('COM_COMMUNITY_VIDEOS_FROM_PAGE','<a href="' . CUrlHelper::pageLink($page->id) . '">' . $page->name. '</a>');
