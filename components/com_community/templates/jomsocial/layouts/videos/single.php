...
<div class="joms-page">
    <div class="joms-list__search">
        ...
        <div class="joms-list__utilities">
            ...
            if($my->authorise('community.create', 'videos') && ($video->creator == $my->id)) { ?>
    ...
    <?php if ($enableReaction && $config->get('likes_videos') && $config->get('enablewalllikereaction')): ?>
        <?php if ($my->id): ?>
        <?php 
        ...
        <div class="joms-stream__actions joms-videos__reaction--btn">
            <?php echo CLikesHelper::renderReactionButton('page', 'videos', $video->id, $reactId) ?>

            <?php if ($config->get('enablesharethis') == 1): ?>
                <a href="javascript:;" onclick="joms.api.pageShare('<?php echo $video->getPermalink(); ?>')">
                    <?php echo JText::_('COM_COMMUNITY_SHARE'); ?>
                </a>
            <?php endif ?>

            <?php if ($enableReporting): ?>
                <a href="javascript:;" onclick="joms.api.videoReport('<?php echo $video->id ?>');">
                    <?php echo JText::_('COM_COMMUNITY_REPORT') ?>
                </a>
            <?php endif ?>

            <?php if (COwnerHelper::isCommunityAdmin() || ($my->id == $video->creator)): ?>
                <a class="joms-js--btn-tag" href="javascript:;"><?php echo JText::_('COM_COMMUNITY_TAG_THIS_VIDEO'); ?></a>
            <?php endif ?>
        </div>

        <div class="joms-stream__status" <?php echo $show ?>>
            <?php echo $like->showWhoReacts('videos', $video->id) ?>
        </div>
        <?php endif ?>
    <?php else: ?>
        <?php if ($config->get('likes_videos') && $config->get('enablewalllikereaction')) { ?>
        <button class="joms-button--small joms-button--<?php echo $likeLiked ? 'primary' : 'neutral' ?> joms-js--like-videos-<?php echo $video->id; ?>"
            ...
            echo JText::_( $likeLiked ? 'COM_COMMUNITY_LIKED' : 'COM_COMMUNITY_LIKE'); ?><?php echo $likeCountHTML; ?>
        </button>
        <?php } ?>
            ...
            <div class="joms-js--video-map-wrapper">
                <div class="app-box-content event-description">
                    ...
                    if (CFactory::getConfig()->get('maps_api', '') == 'openstreetmap') { 
                        echo CMapping::drawOSMMap($video->location, '100%', '100%'); 
                    } else { ?>
                ...
                <div class="joms-module__footer">
                    ...
                    <?php } else { ?>
                        <a href="https://www.openstreetmap.org/?mlat=<?php echo $video->latitude ?>&mlon=<?php echo $video->longitude ?>#map=9/<?php echo $video->latitude ?>/<?php echo $video->longitude ?>" target="_blank" class="joms-button--link"><?php echo JText::_('COM_COMMUNITY_EVENTS_FULL_MAP'); ?></a>
                    <?php } ?>
                </div>
            </div>
            <script>
...
<?php } ?>
        ...
        <div class="cMedia-Author">
            ...
            <a href="<?php echo CRoute::_('index.php?option=com_community&view=videos&catid=' . $video->category_id); ?>">
        ...
        <div>
            ...
            <div class="joms-js--desc-excerpt"><?php echo nl2br(CActivities::format($excerpt)); ?></div>
        ...
        <?php if (($wallCount > 0 || !empty($wallForm)) && $config->get('enablewallcomment')) { ?>
        <br>
        <div>
