...
<div class="joms-main">
    <div class="joms-page">
        ...
        <div class="joms-actions-wrap">
            <?php if ($isLikeEnabled) { ?>
            <!-- like button -->
            <button class="joms-button--smallest joms-js--like-discussion-<?php echo $discussion->id; ?> <?php echo $isUserLiked > 0 ? 'joms-button--primary' : 'joms-button--neutral' ?>"
                    onclick="joms.api.page<?php echo $isUserLiked > 0 ? 'Unlike' : 'Like' ?>('discussion', '<?php echo $discussion->id; ?>');"
                    data-lang-like="<?php echo JText::_('COM_COMMUNITY_LIKE'); ?>"
                    data-lang-liked="<?php echo JText::_('COM_COMMUNITY_LIKED'); ?>">
                <span class="joms-js--lang"><?php echo ($isUserLiked > 0) ? JText::_('COM_COMMUNITY_LIKED') . ' (' . $totalLikes . ')' : JText::_('COM_COMMUNITY_LIKE'); ?></span>
            </button>
            <?php } ?>

            <?php if ($config->get('enablesharethis') == 1) { ?>
            <!-- share button -->
            <button class="joms-button--smallest joms-button--primary" onclick="joms.api.pageShare('<?php echo CRoute::getURI(); ?>')"><?php echo JText::_('COM_COMMUNITY_SHARE'); ?></button>
            <?php } ?>
        </div>
        ...
        <div class="joms-gap"></div>
        <h5 class="joms-text--title"><?php if ($wallCount) echo JText::_('COM_COMMUNITY_REPLIES'); ?></h5>
