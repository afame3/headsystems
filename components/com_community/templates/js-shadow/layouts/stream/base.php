...
$filterValue = JFactory::getApplication()->input->getString('value', $filterValue);
...
$newestStreamId = $activitiesLib->getNewestStreamId($filter, $filterId, $filterValue);
...
<div class="joms-stream__wrapper">
    ...
    <div class="joms-stream__container" data-filter="<?php echo $filter; ?>" data-filter-value="<?php echo $filterValue; ?>" data-filterid="<?php echo $filterId; ?>" data-groupid="<?php $this->groupId; ?>" data-eventid="<?php $this->eventId; ?>" data-profileid="<?php $this->profileId; ?>">
        ...
        <?php foreach ($activities as $act): ?>
            ...
            <div class="joms-stream <?php echo "joms-embedly--" . $config->get('enable_embedly_card_position'); ?> joms-js--stream joms-js--stream-<?php echo $act->id; ?> <?php echo (isset($act->isFeatured) && $act->isFeatured) ? 'joms-stream--featured' : ''; ?>" data-stream-id="<?php echo $act->id; ?>" data-stream-type="<?php echo $act->app; ?>"><div class="joms-stream-inner">
                <?php
                ...
                switch ($act->app) {
                    ...
                    case 'photos.comment':
                        ...
                        break;
                    case 'photos':
                        // do not count unpublished photos
                        $photoIds = $act->params->get('photosId', '');
                        ...
                        foreach ($photoIds as $photoId) {
                            ...
                            if ($photoId) {
                                ...
                                if ($photo->published && $photo->status != 'delete' && $photo->id) {
                                    $photos[] = $photo->id;
                                }
                            }
                        }

                        if (!count($photos)) break;
                        
                        $this->set('count', count($photos));
                        $this->load('stream/photos');
                        break;
                    ...
                    default:
                        ...
                        if (!is_null($plugin)) {
                            ...
                            //dont show footer(likes and comments if this comes from third party), we determine by the type because it will always start with service
                            $app = explode('.', $act->app);
                            if (isset($app[0]) && $app[0] == 'service') {
    ...
    <script>
        ...
        joms_newest_stream_id = +'<?php echo $newestStreamId; ?>';
    </script>
    <?php } ?>
