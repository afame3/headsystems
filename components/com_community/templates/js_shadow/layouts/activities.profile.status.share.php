...
if (!is_null($activity->id)) {
    echo '<div class="joms-stream-inner">';
    ...
    $stream->actor = $user;
    $stream->target = null;
    $stream->headline = JText::sprintf('COM_COMMUNITY_ACTIVITY_SHARE_STATUS', CUrlHelper::userLink($user->id), $user->getDisplayName(false, true), CUrlHelper::userLink($actor->id), $actor->getDisplayName(false, true), (isset($app[$activity->app])) ? $app[$activity->app] : '').$groupString;
    $stream->message = CActivities::format($act->title);
    $stream->rawmessage = $act->title;
    $stream->mood = $params->get('mood', NULL);
    $stream->sharedMessage = $activity->title;
    $stream->sharedMood = $activityParam->get('mood', NULL);
    $stream->backgroundid = $activityParam->get('bgid', NULL);
