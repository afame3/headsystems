    ...
    class CommunityViewPhotos extends CommunityView {
        ...
        public function _flashuploader() {
            ...
            if (!empty($groupId)) {
                ...
                $page->load($groupId);
                $albums = $model->getPageAlbums($pageId, false, false, '', ( $page->isAdmin($my->id) || COwnerHelper::isCommunityAdmin()));
                $createAlbumLink = CRoute::_('index.php?option=com_community&view=photos&task=newalbum&pageid=' . $pageId);
                $photoUploaded = $model->getPhotosCount($pageId, PHOTOS_PAGE_TYPE);
                $photoUploadLimit = $config->get('pagephotouploadlimit');
                $viewAlbumLink = CRoute::_('index.php?option=com_community&view=photos&task=album&pageid=' . $pageId . '&albumid=' . $albumId);
            } else {
        ...
        public function _htmluploader() {
            ...
            if (!empty($groupId)) {
                ...
                $page = JTable::getInstance('Group', 'CTable');
                $page->load($pageId);
                $albums = $model->getPageAlbums($pageId, false, false, '', ( $page->isAdmin($my->id) || COwnerHelper::isCommunityAdmin()));
                $createAlbumLink = CRoute::_('index.php?option=com_community&view=photos&task=newalbum&pageid=' . $pageId);
                $photoUploaded = $model->getPhotosCount($pageId, PHOTOS_PAGE_TYPE);
        ...
        public function display($tpl = null, $albumType = 'special') {
            ...
            if (!empty($pageId)) {
                ...
                if ($params->get('photopermission', 0) <= 0) {
                    $mainframe->redirect(CRoute::_('index.php?option=com_community&view=pages&task=viewpagep&pageid='.$pageId, false));
                }
                
                $pathway = $mainframe->getPathway();
                $pathway->addItem(JText::_('COM_COMMUNITY_PAGES'), CRoute::_('index.php?option=com_community&view=pages'));
                $pathway->addItem($page->name, CRoute::_('index.php?option=com_community&view=pages&task=viewpage&pageid=' . $pageId));

                /**
                 * Opengraph
                 */
                CHeadHelper::setType('website', JText::sprintf('COM_COMMUNITY_PAGES_PHOTO_LISTING',$page->name));
            ...
            }elseif($pageId || $albumType == PHOTOS_PAGE_TYPE){
            ...
            foreach ($albumsData['data'] as $album) {
                if($album->type == "page"){
                    ...
                    if($page->approvals == 1 ){
                        if($page->isMember($my->id) || CFactory::getUser()->authorise('community.pageedit', 'com_community') || CFactory::getUser()->authorise('community.pageeditstate', 'com_community') || CFactory::getUser()->authorise('community.pagedelete', 'com_community'))
                            $albumList[] = $album;
                    } else {
                        $albumList[] = $album;
                    }
                } else if($album->type == "group"){
                    $group = JTable::getInstance('Group', 'CTable');
                    $group->load($album->groupid);

                    if($group->approvals == 1 ){
                        if($group->isMember($my->id) || CFactory::getUser()->authorise('community.groupedit', 'com_community') || CFactory::getUser()->authorise('community.groupeditstate', 'com_community') || CFactory::getUser()->authorise('community.groupdelete', 'com_community'))
                            $albumList[] = $album;
                    } else {
                        $albumList[] = $album;
                    }
        ...
        public function _getAllAlbumsHTML($albums, $type = PHOTOS_USER_TYPE, $pagination = NULL, $isMyOwnPhoto = false) {
            ...
            for ($i = 0; $i < count($albums); $i++) {
                ...
                switch($albums[$i]->type){
                    case 'page.avatar':
                        ...
                        $table->load($albums[$i]->pageid);
                        $tmpName = JText::sprintf('COM_COMMUNITY_PAGE_AVATAR_NAME', ucfirst($table->name));
                    ...
                    case 'page.Cover':
                        $table = JTable::getInstance('Page', 'CTable');
                        $table->load($albums[$i]->pageid);
                        $tmpName = JText::sprintf('COM_COMMUNITY_PAGE_COVER_NAME', ucfirst($table->name));
                        if($albums[$i]->name != $tmpName){
                            $photoModel->updateAlbumName($albums[$i]->id, $tmpName);
                            $albums[$i]->name = $tmpName;
                        }
                    ...
                    case 'page':
                        ...
                        if($albums[$i]->default == 1){
                            $table = JTable::getInstance('Page', 'CTable');
                            $table->load($albums[$i]->pageid);
                            $tmpName = JText::sprintf('COM_COMMUNITY_PAGE_DEFAULT_ALBUM_NAME', ucfirst($table->name));
                ...
                $albumLike = new CLike();
                $albums[$i]->likeCount = $albumLike->getLikeCount('album', $albums[$i]->id);

                //$albums[$i]->totalPhotos = $photosModel->getTotalPhotos($albums[$i]->id);
                if ($type == PHOTOS_PAGE_TYPE) {
                    $albums[$i]->link = CRoute::_("index.php?option=com_community&view=photos&task=album&albumid={$albums[$i]->id}&pageid={$albums[$i]->pageid}");
                    $albums[$i]->editLink = CRoute::_("index.php?option=com_community&view=photos&task=editAlbum&albumid={$albums[$i]->id}&pageid={$albums[$i]->pageid}&referrer={$displaygrp}");
                    $albums[$i]->uploadLink = "javascript:joms.notifications.showUploadPhoto({$albums[$i]->id},{$albums[$i]->pageid});"; 

                    $page = JTable::getInstance('Page', 'CTable');
                    $page->load($pageId);

                    $albums[$i]->isOwner = $page->isAdmin($my->id);
                } else if ($type == PHOTOS_GROUP_TYPE) {
                    ...
                    $albums[$i]->isOwner = $group->isAdmin($my->id);
                } else {
            ...
            if ($type == PHOTOS_PAGE_TYPE) {
                $isOwner = CPageHelper::allowManagePhoto($pageId);
                $baselink = "index.php?option=com_community&view=photos&pageid=".$pageId;
                ...
                $baselink = "index.php?option=com_community&view=photos&groupid=".$groupId;
        ...
        public function _getAlbumsHTML($albums, $type = PHOTOS_USER_TYPE, $pagination = NULL) {
            ...
            $pageId = $jinput->request->get('groupid', '', 'INT');
            ...
            foreach ($albums as &$album) {
                ...
                } else if ($type == PHOTOS_PAGE_TYPE) {
                    $album->link = CRoute::_("index.php?option=com_community&view=photos&task=album&albumid={$album->id}&pageid={$album->pageid}");
                    $album->editLink = CRoute::_("index.php?option=com_community&view=photos&task=editAlbum&albumid={$album->id}&pageid={$album->pageid}&referrer=myphotos");
                    $album->uploadLink = CRoute::_("index.php?option=com_community&view=photos&task=uploader&albumid={$album->id}&pageid={$album->pageid}");
                    $albums->isOwner = $my->authorise('community.view', 'photos.group.album.' . $pageId, $album);
                }
            ...
            if ($type == PHOTOS_PAGE_TYPE) {
                $createLink = CRoute::_('index.php?option=com_community&view=photos&task=newalbum&pageid=' . $pageId);
                $baselink = "index.php?option=com_community&view=photos&pageid=".$pageId;
                $isOwner = CGroupHelper::allowManagePhoto($pageId);
            } else if ($type == PHOTOS_GROUP_TYPE) {
        ...
        public function editAlbum($bolSaveSuccess = true) {
            ...
            if (strpos($album->type, 'page') !== false) {
                $type = PHOTOS_PAGE_TYPE;
            } else if (strpos($album->type, 'group') !== false) {
                $type = PHOTOS_GROUP_TYPE;
            } else {
                $type = PHOTOS_USER_TYPE;
            }
        ...
        public function album() {
            ...
            $jinput->set('pageid',$album->pageid);
            ...
            if($album->type == 'page'){
                if(!$my->authorise('community.view', 'photos.page.album.'.$album->pageid, $album)){
                    $tmpl = new CTemplate();
                    echo $tmpl->fetch('album/missingalbum');
                    return;
                }
            ...
            if ($pageId > 0) {
                ...
                $pathway->addItem(JText::_('COM_COMMUNITY_PAGES'), CRoute::_('index.php?option=com_community&view=pages'));
                $pathway->addItem($page->name, CRoute::_('index.php?option=com_community&view=pages&task=viewpage&pageid=' . $pageId));
                $pathway->addItem(JText::_('COM_COMMUNITY_PHOTOS'), CRoute::_('index.php?option=com_community&view=photos&pageid=' . $pageId));
            } elseif ($groupId > 0) {
                $group = JTable::getInstance('Group', 'CTable');
                $group->load($groupId);
            ...
            if ($pageId > 0) {
                $otherAlbums = $model->getPageAlbums($pageId);
            } elseif ($groupId > 0) {
            ...
            switch(strtolower($album->type)){
                ...
                case 'user':
                    ...
                    break;
                case 'page.cover':
                    $pageTable = JTable::getInstance('Page', 'CTable');
                    $pageTable->load($album->pageid);
                    $album->name = JText::sprintf('COM_COMMUNITY_PHOTOS_ALBUM_DEFAULT_COVER_TITLE', $pageTable->name);
                    $document->setTitle(ucfirst($album->name));
                    break;
                case 'page.avatar':
                    $pageTable = JTable::getInstance('Page', 'CTable');
                    $pageTable->load($album->pageid);
                    $album->name = JText::sprintf('COM_COMMUNITY_PHOTOS_ALBUM_DEFAULT_AVATAR_TITLE', $pageTable->name);
                    $document->setTitle(ucfirst($album->name));
                    break;
                case 'page.gif':
                    $pageTable = JTable::getInstance('Page', 'CTable');
                    $pageTable->load($album->pageid);
                    $album->name = JText::sprintf('COM_COMMUNITY_PHOTOS_ALBUM_DEFAULT_ANIMATION_TITLE', $pageTable->name);
                    $document->setTitle(ucfirst($album->name));
                    break;
                case 'page':
                    $pageTable = JTable::getInstance('Page', 'CTable');
                    ...
                    if($album->default){
                        // this is a stream photos
                        $album->name = JText::sprintf('COM_COMMUNITY_PHOTOS_ALBUM_DEFAULT_STREAM_TITLE',$pageTable->name);
                        $document->setTitle(ucfirst($album->name));
                    }
                    break;
                case 'group.cover':
        ...
        private function _getHandler($type = '') {
            ...
            if ($pageId || $type == 'page') {
                // page photo
                $handler = new CommunityViewPhotosPageHandler($this);
            } elseif ($groupId || $type == 'group') {
                // group photo
...
//		$friends	= $model->getFriends( $this->my->id , '' , false );
        ...
        public function setSubmenus() {
            ...
            switch ($task) {
                case '':
                    ...
                    if($config->get('enablepages')){
                        $this->view->addSubmenuItem('index.php?option=com_community&view=photos&task=page', JText::_('COM_COMMUNITY_PHOTOS_ALL_PAGE_PHOTOS'), '' , false , '' , 'joms-right');
                    }
                    ...
                    $this->view->addSubmenuItem('index.php?option=com_community&view=photos', JText::_('COM_COMMUNITY_PHOTOS_ALL_PHOTOS'), '' , false , '' , 'joms-right');
                ...
                case 'photo':
                    ...
                    if (CFactory::getUser()->authorise('community.photodelete', 'com_community') || ($this->my->id == $album->creator && ($this->my->id != 0))) {
                        $this->view->addSubmenuItem('', JText::_('COM_COMMUNITY_DELETE'), "joms_delete_photo();", true);
                    }

                    if ($this->my->id == $album->creator && ($this->my->id != 0)) {
                        $this->view->addSubmenuItem('', JText::_('COM_COMMUNITY_PHOTOS_SET_AVATAR'),
                                "joms_set_as_profile_picture();", true);
                    }

                    if (CFactory::getUser()->authorise('community.photoedit', 'com_community') || ($this->my->id == $album->creator && ($this->my->id != 0))) {
                ...
                default:
                    ...
                    if (!$albumId && ($this->my->id != 0 || COwnerHelper::isCommunityAdmin())) {
                        $groupid = $jinput->get('groupid', 0, 'INT');
                        $exludeUploadTask = array('newalbum', 'editAlbum', 'display', 'page', 'group', 'event');
                        ...
                        //add a back button
                        if($album->type == 'user' || $album->type == 'profile.gif' || $album->type == 'profile.avatar' || $album->type == 'profile.Cover'){
                            $link = "index.php?option=com_community&view=photos&task=myphotos&userid=".$album->creator;
                        }elseif($album->type == 'page'){
                            $link = "index.php?option=com_community&view=photos&pageid=".$album->pageid;
                        }elseif($album->type == 'group'){
                            $link = "index.php?option=com_community&view=photos&groupid=".$album->groupid;
                        }elseif($album->type == 'event'){
                            $link = "index.php?option=com_community&view=photos&eventid=".$album->eventid;
                        }else{
                            $link = "index.php?option=com_community&view=photos";
                        }
                        $this->view->addSubmenuItem($link, JText::_('COM_COMMUNITY_PHOTOS_BACK_TO_PHOTOS'), '' , false , '' , 'action');
                    }

                    if ($task == 'album' && ($my->id == $album->creator || CFactory::getUser()->authorise('community.photoedit', 'com_community')) && !CAlbumsHelper::isFixedAlbum($album)) {
                    ...
                    if ($task == 'album' && ($my->id == $album->creator || CFactory::getUser()->authorise('community.photodelete', 'com_community')) && !CAlbumsHelper::isFixedAlbum($album)) {
                        $this->view->addSubmenuItem('javascript:void(0);', JText::_('COM_COMMUNITY_PHOTOS_ALBUM_DELETE'), "joms.api.albumRemove('".$albumId."')", true);
                    }

                    if ($task == 'album' && COwnerHelper::isCommunityAdmin() && ($album->permissions == 10 || $album->permissions == 0) && !$album->groupid && !$album->eventid && ($album->type != 'profile.Cover' || $album->type != 'profile.avatar')) {
                        ...
                        if ($config->get('show_featured')) {
                            ...
                            if (!$isFeatured) {
                                $this->view->addSubmenuItem('',
                                    JText::_('COM_COMMUNITY_MAKE_FEATURED'),
                                    'joms.api.albumAddFeatured('.$album->id.')',
                                    true
                                );
                            } else {
                                $this->view->addSubmenuItem('',
                                    JText::_('COM_COMMUNITY_REMOVE_FEATURED'),
                                    'joms.api.albumRemoveFeatured('.$album->id.')',
                                    true
                                );
                            }
                        }
            ...
            $allowMyPhotoTask = array('display','event','group','page','myphotos');
        ...
        public function isAlbumBrowsable($albumId) {
            ...
            // ACL
            if(CFactory::getUser()->authorise('community.photoedit', 'com_community') || CFactory::getUser()->authorise('community.photodelete', 'com_community')) {
                return true;
            }
    ...
    class CommunityViewPhotosGroupHandler extends CommunityViewPhotoHandler {
        ...
        public function __construct($viewObj) {
            ...
            $jinput = JFactory::getApplication()->input;
            $this->groupid = $jinput->get('groupid', 0, 'INT');
        }
    ...
    class CommunityViewPhotosPageHandler extends CommunityViewPhotoHandler {
        ...
        public function __construct($viewObj) {
            ...
            $jinput = JFactory::getApplication()->input;
            $this->pageid = $jinput->get('pageid', 0, 'INT');
        }

        public function getFlashUploadURI($token, $albumId) {
            $session = JFactory::getSession();
            $url = 'index.php?option=com_community&view=photos&task=upload&no_html=1&albumid=' . $albumId . '&tmpl=component';
            $url .= '&' . $session->getName() . '=' . $session->getId() . '&token=' . $token->token . '&uploaderid=' . $this->my->id . '&pageid=' . $this->pageid;
            $url = JURI::root(true). '/' . $url;
            return $url;
        }

        public function getAlbumCreateLink() {
            return CRoute::_('index.php?option=com_community&view=photos&task=newalbum&pageid=' . $this->pageid);
        }
        ...
        public function isTaggable() {
            ...
            //check if we can allow the current viewing user to tag the photos
            if ($page->isMember($this->my->id) || $page->isAdmin($this->my->id) || COwnerHelper::isCommunityAdmin()) {
                return true;
            }
            return false;
        }
        ...
        public function getTaggingUsers() {
            ...
            $ids = $model->getMembersId($this->pageid, true);
            $users = array();
            ...
            if (COwnerHelper::isCommunityAdmin() || $page->isAdmin($this->my->id) || $page->isMember($this->my->id))
                array_unshift($users, $this->my);
        ...
        public function getAlbumPhotos($albumId) {
            ...
            // @todo: make limit configurable?
            return $model->getAllPhotos($albumId, PHOTOS_PAGE_TYPE, null, null, $config->get('photosordering'));
        }
        ...
        public function setSubmenus() {
            ...
            $albumId = $userid = $jinput->get('albumid', 0, 'INT');
            $photoId = $userid = $jinput->get('photoid', 0, 'INT');
            $pageId = $userid = $jinput->get('pageid', 0, 'INT');
            ...
            // Check if the current user is banned from this page
            $isBanned = $page->isBanned($my->id);
            $allowManagePhotos = CPageHelper::allowManagePhoto($this->pageid);

            if (($task == 'uploader' || $task == 'photo' || $task =='album') && !empty($albumId)) {
                $this->view->addSubmenuItem('index.php?option=com_community&view=photos&pageid=' . $this->pageid . '&task=' . ($photoId > 0 ? 'album' : 'display') . '&albumid=' . $albumId, JText::_('COM_COMMUNITY_PHOTOS_BACK_TO_ALBUM'));
            }

            if ($allowManagePhotos && $task != 'photo' && !$isBanned) {
                /* Page: Upload Photos */
                if ($task == 'album' && ( ($my->id == $album->creator && $allowManagePhotos ) || $page->isAdmin($my->id) || COwnerHelper::isCommunityAdmin() )) {

                    if(!CAlbumsHelper::isFixedAlbum($album)){
                        $this->view->addSubmenuItem('javascript:', JText::_('COM_COMMUNITY_PHOTOS_ALBUM_DELETE'), "joms.api.albumRemove('" . $album->id . "', '" . $task . "');", true);
                        $this->view->addSubmenuItem('index.php?option=com_community&view=photos&task=editAlbum&albumid=' . $album->id . '&pageid=' . $page->id . '&referrer=albumpg', JText::_('COM_COMMUNITY_EDIT_ALBUM'), '', true);
                    }
                }
            }
            ...
            if ($task == 'photo') {
                ...
                if (($my->id == $album->creator && $allowManagePhotos ) || $page->isAdmin($my->id) || COwnerHelper::isCommunityAdmin()) {
                    $this->view->addSubmenuItem('', JText::_('COM_COMMUNITY_PHOTOS_SET_AS_ALBUM_COVER'), "joms_set_as_album_cover();", true);
                }
                ...
                if ($pageId != '' && $task=='myphotos') {
                    $this->view->addSubmenuItem('index.php?option=com_community&view=photos&task=newalbum&pageid=' . $pageId, JText::_('COM_COMMUNITY_PHOTOS_CREATE_PHOTO_ALBUM'), '', true);
                } elseif($task=='myphotos') {
                    $this->view->addSubmenuItem('index.php?option=com_community&view=photos&task=newalbum&userid=' . $my->id, JText::_('COM_COMMUNITY_PHOTOS_CREATE_PHOTO_ALBUM'), '', true);
              }
            }
        ...
        public function isAlbumBrowsable($albumId) {
            ...
            //@rule: Do not allow non members to view albums for private page
            if ($page->approvals == COMMUNITY_PRIVATE_PAGE && !$page->isMember($this->my->id) && !$page->isAdmin($this->my->id) && !COwnerHelper::isCommunityAdmin()) {
                /**
                 * Opengraph
                 */
                CHeadHelper::setType('website', JText::_('COM_COMMUNITY_RESTRICTED_ACCESS'));
                ...
                echo JText::_('COM_COMMUNITY_PAGES_ALBUM_MEMBER_PERMISSION');
                return false;
            }
         ...
         */
        public function getAlbumURI($albumId) {
            return CRoute::_('index.php?option=com_community&view=photos&task=album&albumid=' . $albumId . '&pageid=' . $this->pageid);
        }

        public function getAlbumExternalURI($albumId) {
            return CRoute::getExternalURL('index.php?option=com_community&view=photos&task=album&albumid=' . $albumId . '&pageid=' . $this->pageid);
        }

        public function getPhotoURI($photoId, $albumId) {
            return CRoute::_('index.php?option=com_community&view=photos&task=photo&pageid=' . $this->pageid . '&albumid=' . $albumId . '&photoid=' . $photoId);
        }

        public function getPhotoExternalURI($photoId, $albumId) {
            return CRoute::getExternalURL('index.php?option=com_community&view=photos&task=photo&albumid=' . $albumId . '&pageid=' . $this->pageid . '&photoid=' . $photoId);
        }
        ...
        public function isExceedUploadLimit() {
            if (CLimitsHelper::exceededPhotoUpload($this->pageid, PHOTOS_PAGE_TYPE)) {
                ...
                echo JText::sprintf('COM_COMMUNITY_PAGES_PHOTO_LIMIT', $photoLimit);
                return TRUE;
            }
        ...
        public function getAllAlbumData() {
            ...
            if(!$this->pageid){
                // if there is no id supplied, this should be listing all the page album that is visible to the current user
                $excludeType = array(
                    'page.default',
                    'page.gif',
                    'page.stream',
                ...
                $pageModel = CFactory::getModel('pages');
                $pagesId = $pageModel->getUserViewablepages(CFactory::getUser()->id);
                $albumsData['data'] = $this->model->getPageAlbums($pagesId, $type, false, '', false, '', $excludeType);
                $albumsData['pagination'] = $this->model->getPagination();
                return $albumsData;
            }
            ...
            $albumsData['data'] = $this->model->getPageAlbums($this->pageid, $type);
            $albumsData['pagination'] = $this->model->getPagination();
        ...
        public function setMiniHeader() {
            // Do nothing because the mini header for pages are done on the view itself. Function is to satisfy the abstract.
        }
...
//		return $uri->toString();
        ...
        public function setSubmenus() {
            ...
            if (($task == 'uploader' || $task == 'photo' || $task == 'album') && !empty($albumId)) {
                $this->view->addSubmenuItem('index.php?option=com_community&view=photos&eventid=' . $this->eventid . '&albumid=' . $albumId, JText::_('COM_COMMUNITY_PHOTOS_BACK_TO_ALBUM'));
            }
            ...
            if ($allowManagePhotos && $task != 'photo' && !$isBanned) {
                ...
                if ($task == 'album' && ( ($my->id == $album->creator && $allowManagePhotos ) || $event->isAdmin($my->id) || COwnerHelper::isCommunityAdmin() || CFactory::getUser()->authorise('community.photodelete', 'com_community'))) {
                ...
                if ($task == 'album' && ( ($my->id == $album->creator && $allowManagePhotos ) || $event->isAdmin($my->id) || COwnerHelper::isCommunityAdmin() || CFactory::getUser()->authorise('community.photoedit', 'com_community'))) {
        ...
        public function getAllAlbumData() {
            ...
            //check if this event belongs to a page
            if($event->type == 'page' && $event->contentid){
                $pageid = $event->contentid;
                $page = JTable::getInstance('Page', 'CTable');
                $page->load($pageid);

                //@rule: Do not allow non members to view albums for private page
                if ($page->approvals == COMMUNITY_PRIVATE_PAGE && !$page->isMember($my->id) && !$page->isAdmin($my->id)) {
                    $this->noAccess();
                    return FALSE;
                }
            }
