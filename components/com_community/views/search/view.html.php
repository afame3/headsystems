...
//                $this->addSubmenuItem('index.php?option=com_community&view=search', JText::_('COM_COMMUNITY_SEARCH_FRIENDS'), 'joms.videos.toggleSearchSubmenu(this)', SUBMENU_LEFT, $html);
        ...
        public function browse($data = null) {
            ...
            $filter = $jinput->get->get('filter', 'all', 'STRING');
            $radius = $jinput->getInt('radius', 0);
            $profiletype = $jinput->getInt('profiletype', 0);

            $rows = $searchModel->getPeople($sorted, $filter, $profiletype, $radius);
            ...
            if(!CFactory::getConfig()->get('use_gravatar')){
                $sortItems = array_merge(array('avatar' => JText::_('COM_COMMUNITY_SORT_AVATAR')),$sortItems);
            }
            ...
            $radiusItems = array();
            if ($config->get('advanced_search_units')){
                $radiusItems = array(10, 20, 50, 100, 200, 500);
            }
            ...
            $radiusHTML = '';
            if ($config->get('memberlist_show_distance') && CMapsHelper::mapSetup()) {
                $radiusHTML = CFilterBar::getHTML(CRoute::getURI(), '','', '', '', $radiusItems);
            }            
        ...
        public function field($data) {
            ...
            $pageTitle = JText::sprintf('COM_COMMUNITY_MEMBERS_WITH_FIELD', JText::_($profileName), JText::_($vals[0]));
            CHeadHelper::setType('website', $pageTitle);
        ...
        public function advanceSearch() {
            ...
            $simpleSearch = $jinput->get('q', 0, 'RAW');
            if($simpleSearch){
                $itemId = $mainframe->getMenu()->getActive()->id;
                //simple search will search for the names only, we will forge this
                $mainframe->redirect(CRoute::_('index.php?field0=username&condition0=contain&value0='.$simpleSearch.'&fieldType0=text&operator=and&profiletype=0&option=com_community&view=search&task=advancesearch&key-list=0&Itemid='.$itemId, false));
            }
            ...
            if (strlen($keyList) > 0) {
                ...
                foreach ($key as $idx) {
                    ...
                    $obj->condition = isset($post['condition' . $idx]) ? $post['condition' . $idx] : '=';
                    ...
                    // if yes and as a date, make them an array.
                    if (isset($post['value' . $idx . '_2'])) {
                        if (($obj->fieldType == 'date' || $obj->fieldType == 'birthdate') && !preg_match('/^[0-9]*$/', $post['value' . $idx . '_2'])) {
                        ...
                        if (($obj->fieldType == 'date' || $obj->fieldType == 'birthdate') && !preg_match('/^[0-9]*$/', $post['value' . $idx])) {
                            $startDate = (empty($post['value' . $idx])) ? '01/01/1970' : $post['value' . $idx];
                            $startDate = date('Y-m-d', strtotime($startDate));
                            $delimeter = '-';
                            if (strpos($startDate, '/')) {
            ...
            if($config->get('profile_multiprofile')){
                ...
                if($profiles){
                    $multiprofileArr[] =  array(
                        ...
                        'name' => JText::_('COM_COMMUNITY_ALL_MEMBERS'),
                        'selected' => (!$profileType) ? 1 : 0
                    );
                    foreach($profiles as $profile){
                        $multiprofileArr[] = array(
